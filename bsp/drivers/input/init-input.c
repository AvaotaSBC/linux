...
static int sunxi_motor_startup(enum input_sensor_type *motor_type)
{
	...
	if (ret) {
		...
		pr_err("Set the default voltage to 3300mv!\n");
		data->ldo_voltage = 3300;
	}

	if (!IS_ERR(data->motor_power_ldo) || !ret) {
		regulator_set_voltage(data->motor_power_ldo,
				(int)(data->ldo_voltage) * 1000,
				(int)(data->ldo_voltage) * 1000);
	}
...
/******************************** MAGENTIC SENSOR START ***********************/

static int sunxi_mag_sensor_startup(enum input_sensor_type *mag_type)
{
	int ret = -1;
	struct sensor_config_info *data = container_of(mag_type,
					struct sensor_config_info, input_type);
	struct device_node *np = NULL;
	struct i2c_client *client = NULL;
	struct platform_device *pdev = NULL;
	...
	if (!np) {
		if (data->np_name != NULL) {
			...
			np = of_find_node_by_name(NULL, "magnetometer");
		}
	}

	if (!np) {
		pr_err("ERROR! get mag sensor_para failed, func:%s, line:%d\n",
							__func__, __LINE__);
		goto devicetree_get_item_err;
	}

	if (!of_device_is_available(np)) {
		pr_err("%s: mag sensor is not used\n", __func__);
		goto devicetree_get_item_err;
	} else
		data->sensor_used = 1;
	...
	if (data->sensor_used == 1) {
		ret = of_property_read_u32(np, "mag_twi_id", &data->twi_id);
		if (ret) {
			pr_err("get mag sensor_twi_id is fail, %d\n", ret);
			goto devicetree_get_item_err;
		}
	} else
		pr_err("%s mag sensor_unused\n", __func__);

	ret = of_property_read_u32(np, "mag_vcc_io_val",
						&data->sensor_power_vol);
	if (ret) {
		pr_err("get mag sensor_vcc_io_val is fail, %d\n", ret);
		goto devicetree_get_item_err;
	}
...
devicetree_get_item_err:
	pr_notice("=========mag sensor script_get_err============\n");
	ret = -1;
	return ret;
}

static int sunxi_mag_sensor_init(enum input_sensor_type *mag_type)
{
	struct sensor_config_info *data = container_of(mag_type,
					struct sensor_config_info, input_type);

	data->sensor_power_ldo = regulator_get(data->dev, "mag");
	if (!IS_ERR_OR_NULL(data->sensor_power_ldo)) {
		regulator_set_voltage(data->sensor_power_ldo,
				(int)(data->sensor_power_vol)*1000,
				(int)(data->sensor_power_vol)*1000);
...
static void sunxi_mag_sensor_free(enum input_sensor_type *mag_type)
{
	struct sensor_config_info *data = container_of(mag_type,
					struct sensor_config_info, input_type);
