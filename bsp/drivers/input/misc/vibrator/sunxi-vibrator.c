...
static void set_sunxi_vibrator(int on)
{
	...
	//	}
	//} else {
	if (IS_ERR_OR_NULL(motor_info.motor_power_ldo) || motor_info.ldo_voltage < 0) {
		pr_err("%s: power_ldo or ldo_voltage not configured!\n", __func__);
		return;
	}

	if (on) {
		if (regulator_is_enabled(motor_info.motor_power_ldo) == 0) {
			regulator_set_voltage(motor_info.motor_power_ldo,
		...
		if (regulator_is_enabled(motor_info.motor_power_ldo)) {
			regulator_set_voltage(motor_info.motor_power_ldo,
								0, (int)(motor_info.ldo_voltage) * 1000);
...
static void vibrator_enable(int value)
{
	mutex_lock(&lock);
	if (vibe_state == 1) {
...
static int vibrator_get_time(void)
{
	struct timespec64 time_tmp;
	if (hrtimer_active(&vibe_timer)) {
		ktime_t r = hrtimer_get_remaining(&vibe_timer);
		time_tmp = ktime_to_timespec64(r);
	       ...
	       const char *buf, size_t count)
{
	...
	vibrator_enable(value);

	return count;
}

static CLASS_ATTR_RW(on);

static struct attribute *vibrator_class_attrs[] = {
	&class_attr_on.attr,
	NULL
};
ATTRIBUTE_GROUPS(vibrator_class);
