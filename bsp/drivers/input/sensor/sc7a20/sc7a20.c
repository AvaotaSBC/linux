				   ...
				   const struct i2c_device_id *id)
{
	...
	result = gsensor_i2c_test(sc7a20_i2c_client);
	if (!result) {
		input_set_power_enable(&(gsensor_info.input_type),  0);
		dev_err(&client->dev, "%s:I2C connection might be something wrong or maybe the other gsensor equipment! \n", __func__);
		return -ENODEV;
	}
...
static void sc7a20_shutdown(struct i2c_client *client)
{
	printk("sc7a20 shut down\n");
