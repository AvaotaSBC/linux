...
#include "gslX680_3676_1280x800_yhk.h"
...
struct gslX680_fw_array gslx680_fw_data[] = {
	...
	{"gslX680_3676_1280x800_yhk", ARRAY_SIZE(GSLX680_FW_3676_1280x800_yhk), GSLX680_FW_3676_1280x800_yhk},
	{"gslX680_5680_d10", ARRAY_SIZE(GSLX680_FW_5680_d10), GSLX680_FW_5680_d10},
};

unsigned int *gslX680_fw_config_data[] = {
	gsl_config_data_id_3676_855280,
	gsl_config_data_id_3676_1280x800_yhk,
	gsl_config_data_id_5680_d10,
...
static int gsl_find_fw_idx(void)
{
	int i = 0;

	if (config_info.name != NULL) {
		for (i = 0; i < ARRAY_SIZE(gslx680_fw_data); i++) {
			if (!strcmp(config_info.name, gslx680_fw_data[i].name)) {
				gslx680_fw_grp = gslx680_fw_data[i];
				gslX680_config_data = gslX680_fw_config_data[i];
				return i;
			}
		}
	}
...
static void gsl_ts_xy_worker(struct work_struct *work)
{
	...
	if (read_buf[3] == 0 && read_buf[2] == 0 && read_buf[1] == 0 &&
		...
		printk("gsl read data error, reinit chip!\n");
	}
...
static void glsX680_resume_events(struct work_struct *work)
{
	...
	del_timer(&monitor_timer);
#endif
	set_bit(PANEL_RESETTING, &ts_init->flags);
	while (retry > 0) {
		gslX680_shutdown_low();
		if (ldo_state) {
	...
	clear_bit(PANEL_RESETTING, &ts_init->flags);
}
...
static int gsl_ts_suspend(struct device *dev)
{
	...
	if (probe_state == 0) {
		pr_err("gsl probe init undone; blocked suspend until proble completed\n");
		return -1;
	}
...
static int gsl_ts_resume(struct device *dev)
{
	...
	if (probe_state == 0) {
		pr_err("gsl probe init undone; ignore resume until proble completed\n");
		return 0;
	}
...
static ssize_t gsl_idle_enable_store_l(const char *buf)
{
	...
	if (probe_state == 0) {
		pr_err("gsl probe init undone; ignore idle change until proble completed\n");
		return 0;
	}
			...
			const struct i2c_device_id *id)
{
	...
	if (!ctp_i2c_test(client)) {
		dev_err(&client->dev, "I2C test fail\n");
		return -ENODEV;
	}
...
#endif /* GSL_MONITOR_TIMER_ENABLE */
