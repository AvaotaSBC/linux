	       ...
	       const char *buf, size_t count)
{
	...
	if ('1' == buf[0]) {
		dev_dbg(&data->client->dev, "suspend by tp idle");
		data->tp_idle_status = 1;
		gtp_suspend(data);
	} else if ('0' == buf[0]) {
		dev_dbg(&data->client->dev, "Resume by tp idle");
		data->tp_idle_status = 0;
		gtp_resume(data);
	} else {
		dev_err(&data->client->dev, "Invalid argument for tp idle\n");
	}
	return count;
}
...
static int gtp_create_file(struct goodix_ts_data *ts)
{
	...
	gtp_config_proc = proc_mkdir("gt9xx_fw", NULL);
	if (!gtp_config_proc)
		return -ENODEV;

	/*  Create proc file system */
	entry = proc_create(GT91XX_CONFIG_PROC_FILE, 0664,
				      gtp_config_proc, &config_proc_ops);
	if (!entry)
	...
	ret = sysfs_create_group(&ts->input_dev->dev.kobj, &gtp_attr_group);
	if (ret) {
	...
	if (ts->pdata->tp_idle_support) {
		ret = class_register(&ctp_class);
		if (ret) {
			dev_err(&client->dev, "Failure create sysfs group for ctp %d\n", ret);
		}
	}
			...
			struct goodix_ts_platform_data *pdata)
{
	...
	of_property_read_u32(np, "goodix,send-cfg-id",
			     &pdata->send_cfg_id);
	if (pdata->send_cfg_id >= 0)
		dev_info(dev, "driver-send-cfg id %d\n", pdata->send_cfg_id);

	of_property_read_u32(np, "goodix,swap-x2y", &pdata->swap_x2y);
	if (pdata->swap_x2y)
		dev_info(dev, "swap-x2y enabled\n");

	of_property_read_u32(np, "goodix,revert_x", &pdata->revert_x);
	if (pdata->revert_x)
		dev_info(dev, "revert_x enabled\n");

	of_property_read_u32(np, "goodix,revert_y", &pdata->revert_y);
	if (pdata->revert_y)
		dev_info(dev, "revert_y enabled\n");

	of_property_read_u32(np, "goodix,tp_idle_support", &pdata->tp_idle_support);
	if (pdata->tp_idle_support)
		dev_info(dev, "tp_idle enabled\n");
	...
	ret = of_property_read_u32(np, "goodix,report-stylus",
			     &pdata->report_stylus);
	if (ret)
		pdata->report_stylus = false;

	if (pdata->report_stylus)
		dev_info(dev, "report_stylus enabled\n");
...
static int gtp_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	if (!gUseDetect && startup()) {
		dev_err("can not startup device!\n");
		return -ENOMEM;
	}
#endif
#endif
...
static int gtp_drv_remove(struct i2c_client *client)
{
	...
	remove_proc_entry(GT91XX_CONFIG_PROC_FILE, gtp_config_proc);
	remove_proc_entry("gt9xx_fw", NULL);

	sysfs_remove_group(&ts->input_dev->dev.kobj, &gtp_attr_group);
...
static void gtp_resume(struct goodix_ts_data *ts)
{
	...
	if (!test_bit(SLEEP_MODE, &ts->flags)) {
		dev_info(&ts->client->dev, "Already in awake state\n");
		return;
	}
