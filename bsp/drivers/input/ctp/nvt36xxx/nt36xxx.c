 ...
 *
 * $Revision: 120016 $
 * $Date: 2023-06-07 11:42:43 +0800 (周三, 07 6月 2023) $
 *
...
static int32_t nvt_parse_dt(struct device *dev)
{
	...
	ret = of_property_read_u32(np, "novatek,max-size-x", &ts->abs_size_x);
	if (ret) {
		NVT_LOG("unset max size x ,use default\n");
		ts->abs_size_x = TOUCH_MAX_X;
	}
	ret = of_property_read_u32(np, "novatek,max-size-y", &ts->abs_size_y);
	if (ret) {
		NVT_LOG("unset max size y,use default\n");
		ts->abs_size_y = TOUCH_MAX_Y;
	}

	ret = of_property_read_u32(np, "novatek,swap-x2y", &ts->swap_x2y);
	if (ret) {
		ts->swap_x2y = 0;
	}

	ret = of_property_read_u32(np, "novatek,revert_x", &ts->revert_x);
	if (ret) {
		ts->revert_x = 0;
	}

	ret = of_property_read_u32(np, "novatek,revert_y", &ts->revert_y);
	if (ret) {
		ts->revert_y = 0;
	}
   ...
   if (POINT_DATA_LEN >= POINT_DATA_CHECKSUM_LEN) {
	...
	for (i = 0; i < ts->max_touch_num; i++) {
		...
		if (((point_data[position] & 0x07) == 0x01) || ((point_data[position] & 0x07) == 0x02)) {	//finger down (enter & moving)
			...
			if (ts->swap_x2y) {
				TP_SWAP(input_x, input_y);
			}

			if ((input_x > ts->abs_size_x) || (input_y > ts->abs_size_y))
				continue;

			if (ts->revert_x)
				input_x = ts->abs_size_x - input_x;

			if (ts->revert_y)
				input_y = ts->abs_size_y - input_y;
...
*/
		...
		if (pen_format_id != 0xFF) {
			if (pen_format_id == 0x01) {
				...
				if ((pen_x > PEN_MAX_WIDTH) || (pen_y > PEN_MAX_HEIGHT))
					goto XFER_ERROR;
...
static int32_t nvt_ts_probe(struct spi_device *client)
{
	...
	if (ts->swap_x2y) {
		TP_SWAP(ts->abs_size_x, ts->abs_size_y);
	}
	...
	input_set_abs_params(ts->input_dev, ABS_MT_POSITION_X, 0, ts->abs_size_x, 0, 0);
	input_set_abs_params(ts->input_dev, ABS_MT_POSITION_Y, 0, ts->abs_size_y, 0, 0);
#if MT_PROTOCOL_B
	...
	if (ts->pen_support) {
		...
		input_set_abs_params(ts->pen_input_dev, ABS_X, 0, PEN_MAX_WIDTH, 0, 0);
		input_set_abs_params(ts->pen_input_dev, ABS_Y, 0, PEN_MAX_HEIGHT, 0, 0);
...
static int32_t nvt_ts_resume(struct device *dev)
{
	...
	gpio_set_value(ts->reset_gpio, 1);
#endif
	// need at leat 10ms delay after POR(power on reset)
	queue_delayed_work(nvt_fwu_wq, &ts->nvt_fwu_work, 50);
