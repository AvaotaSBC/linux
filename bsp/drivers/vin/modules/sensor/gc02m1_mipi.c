...
#define SENSOR_NUM	0x4
#define SENSOR_NAME 	"gc02m1_mipi"
#define SENSOR_NAME_2	"gc02m1_mipi_2"
#define SENSOR_NAME_3 	"gc02m1_mipi_b"
#define SENSOR_NAME_4	"gc02m1_mipi_f"
...
static int sensor_s_gain(struct v4l2_subdev *sd, unsigned int gain_val)
{
	...
	temp_gain = All_gain * GC02M1_SENSOR_DGAIN_BASE / GC02M1_AGC_Param[gain_index][0];
...
static int sensor_power(struct v4l2_subdev *sd, int on)
{
	...
	case PWR_ON:
		...
		vin_set_pmu_vol(sd, DVDD, VDD_1800MV);
...
static int sensor_init(struct v4l2_subdev *sd, u32 val)
{
	...
	if (sensor_otp_enable == 0) {
		...
		if (ret < 0 || sensor_otp_info[0x0006] != 0x10) {
			sensor_otp_enable = -1;
			sensor_dbg("sensor_otp_disable ret=%d [0x0006]=%x\n",
		...
		if (ret < 0)
			...
			sensor_dbg("sensor ckeck OTP success\n");

		conver_Otp2AwInfo(&sensor_otp_info[0], &sensor_otp_lscinfo[0]);

		sensor_dbg("sensor_otp_lscinfo\n");
		for (i = 0; i < LSC_INFO_SIZE; i++) {
			if (i % 16 == 0)
				sensor_dbg("\n");
			sensor_dbg("%d, ", sensor_otp_lscinfo[i]);
		}
		sensor_dbg("\n");

		sensor_dbg("sensor_otp_AWB\n");
		for (i = 0; i < AWB_INFO_SIZE; i++)
			sensor_dbg("%d, ",
		...
		/* convert_lsc_tbl */
		for (i = 0; i < LSC_INFO_SIZE; i++)
			sensor_otp_lscinfo[i] = convert_lsc_tbl[i] * sensor_otp_lscinfo[i] / 100000;
	}
#endif
...
static long sensor_ioctl(struct v4l2_subdev *sd, unsigned int cmd, void *arg)
{
	...
	case VIDIOC_VIN_SET_SENSOR_OTP_INFO: {
		...
		sensor_dbg("--- VIDIOC_VIN_SET_SENSOR_OTP_INFO ---\n ");
		...
		sensor_dbg("sensor_otp_lscinfo----R:\n");
		for (i = 0; i < LSC_INFO_SIZE; i++) {
			if (i % 16 == 0)
				sensor_dbg("\n");
...
static const struct i2c_device_id sensor_id_4[] = {
	{SENSOR_NAME_4, 0},
	{}
};
...
MODULE_DEVICE_TABLE(i2c, sensor_id_4);
