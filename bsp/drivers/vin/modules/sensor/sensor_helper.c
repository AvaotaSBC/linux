				...
				struct sensor_format_struct **sf)
{
	...
	if (fmt->pad == SENSOR_PAD_SOURCE) {
		...
		if ((info->stream_count > 0 || info->use_current_win) &&
		 info->current_wins != NULL) {
#else
		if ((sd->entity.stream_count > 0 || info->use_current_win) &&
		    info->current_wins != NULL && (!info->tvin.flag)) {
#endif
...
int sensor_set_ir(struct v4l2_subdev *sd, struct ir_switch *ir_switch)
{
	...
	if (ir_switch->ir_hold) { /* boot0 set ir, kernel hold it */
		...
		if (!info->init_timer) {
			...
			timer_setup(&info->timer_for_ir, ir_hold, 0);
...
int sensor_enum_frame_interval(struct v4l2_subdev *sd,
			struct v4l2_subdev_state *state,
			struct v4l2_subdev_frame_interval_enum *fie)
{
	...
	if (fie->index >= info->win_size_num)
		return -EINVAL;

	// enum supported frame-rate at a specified resolution
	for (; fie->index < info->win_size_num; fie->index++) {
		win_size = &info->win_pt[fie->index];

		if (win_size->width == fie->width && win_size->height == fie->height)
			break;
	}
	...
	fie->interval.denominator = info->win_pt[fie->index].fps_fixed;
	return 0;
}
EXPORT_SYMBOL_GPL(sensor_enum_frame_interval);
...
int sensor_s_parm(struct v4l2_subdev *sd, struct v4l2_streamparm *parms)
{
	...
	info->large_image = cp->reserved[2];
	return 0;
}
...
static int sensor_helper_probe(struct platform_device *pdev)
{
	...
	__maybe_unused struct device *dev = &pdev->dev;
	struct sensor_helper_dev *sensor_helper = NULL;
	__maybe_unused char power_supply[20] = {0};
	__maybe_unused char property_name[16] = {0};
	__maybe_unused const char *ccm_name;
	int ret;
	...
	sensor_helper = kzalloc(sizeof(*sensor_helper), GFP_KERNEL);
	if (!sensor_helper) {
		ret = -ENOMEM;
		goto ekzalloc;
	}
