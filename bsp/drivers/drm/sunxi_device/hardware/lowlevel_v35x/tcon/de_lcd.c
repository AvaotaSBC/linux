...
static volatile struct __de_lcd_top_dev_t *lcd_top[DEVICE_NUM];//fixme
#endif

#if defined(SUPPORT_COMBO_DPHY)
extern __s32 lvds_combphy_open(__u32 sel, struct disp_panel_para *panel);
extern __s32 lvds_combphy_close(__u32 sel);
#endif
...
s32 tcon1_edp_clk_enable(u32 sel, u32 en)
{
	...
	if (en) {
		lcd_top[0]->tcon_tv_setup.bits.tv1_clk_src = TV_CLK_F_TVE;
		lcd_top[0]->tcon_clk_gate.bits.tv1_clk_gate = TV_CLK_F_TVE;
		lcd_top[0]->tcon_clk_gate.bits.hdmi_src = 2;
	} else {
		lcd_top[0]->tcon_tv_setup.bits.tv1_clk_src = 0;
		lcd_top[0]->tcon_clk_gate.bits.tv1_clk_gate = 0;
...
s32 tcon1_hdmi_clk_enable(u32 sel, u32 en)
{
	...
	if (en) {
		...
		if (((sel == 2) && (lcd_top[0]->tcon_clk_gate.bits.hdmi_src == 1))) {
			lcd_top[0]->tcon_clk_gate.bits.hdmi_src = 0;
		}
	}
...
s32 tcon_de_attach(u32 tcon_index, u32 de_index)
{
	...
	if (de_index == 0) {
		...
		lcd_top[0]->tcon_de_perh.bits.de_port1_perh =
		    (tcon_index == 1) ? 0 : 2;
#else
		if (lcd_top[0]->tcon_de_perh.bits.de_port1_perh == tcon_index)
			lcd_top[0]->tcon_de_perh.bits.de_port1_perh =
			    lcd_top[0]->tcon_de_perh.bits.de_port0_perh;
		...
		lcd_top[0]->tcon_de_perh.bits.de_port0_perh =
		    (tcon_index == 1) ? 0 : 2;
#else
		if (lcd_top[0]->tcon_de_perh.bits.de_port0_perh == tcon_index)
			lcd_top[0]->tcon_de_perh.bits.de_port0_perh =
			    lcd_top[0]->tcon_de_perh.bits.de_port1_perh;
...
s32 lvds_open(u32 sel, struct disp_panel_para *panel)
{
	...
	if (panel->lcd_lvds_if == LCD_LVDS_IF_DUAL_LINK) {
		...
		/* aw1890 dual lvds: tcon0 -> combophy0 & 1, tcon2 -> lvds_phy*/
		{
			lvds_combphy_open(sel, panel);
			lvds_combphy_open(sel + 1, panel);
		}
#else
			lvds_combphy_open(sel, panel);
#endif
...
s32 tcon0_open(u32 sel, struct disp_panel_para *panel)
{
	...
	if ((panel->lcd_if == LCD_IF_HV) || (panel->lcd_if == LCD_IF_LVDS)
		...
		if (panel->lcd_dsi_if == LCD_DSI_IF_COMMAND_MODE)
			...
			tcon_irq_enable(sel, LCD_IRQ_TCON0_VBLK);
			lcd_dev[sel]->tcon0_dclk.bits.tcon0_dclk_en = 0x2;
		}
#endif
		else
			lcd_dev[sel]->tcon0_dclk.bits.tcon0_dclk_en = 0xf;
...
static s32 tcon0_cfg_mode_tri(u32 sel, struct disp_panel_para *panel)
{
	 ...
	 *
	 */
	if (panel->lcd_start_delay != 0)
		delay_line = panel->lcd_start_delay;
	else if (panel->lcd_vbp > 10)
		delay_line = 10;
...
s32 tcon0_cfg(u32 sel, struct disp_panel_para *panel, u32 de_use_rcq)
{
	if ((panel->lcd_if == LCD_IF_HV) || (panel->lcd_if == LCD_IF_EXT_DSI)) {
		...
		if (panel->lcd_tcon_mode < DISP_TCON_DUAL_DSI) {
			...
			if (panel->lcd_tcon_en_odd_even == 0) {
				/**
				 * TCON divides a frame of image into left and
				 * right halves and sends it to two DSI modules
				 */

				/* enable 3D FIFO */
				lcd_dev[sel]->tcon0_3d_fifo.bits.fifo_3d_setting = 2;
				lcd_dev[sel]->tcon0_3d_fifo.bits.fifo_3d_half_line_size =
					panel->lcd_x / 2 - 1;
				lcd_dev[sel]->tcon0_3d_fifo.bits.fifo_3d_bist_en = 0;
				lcd_dev[sel]->tcon0_3d_fifo.bits.fifo_3d_overlap = 0;
			} else {
				/**
				 * TCON sends a frame of image to two DSI
				 * modules by dividing odd and even dot
				 */
				/* do nothing */
			}
		}

#if defined(SUPPORT_DSI) && !defined(DSI_VERSION_40) || defined(CONFIG_ARCH_SUN55IW3)
...
#endif
		{
			/* dsi command mode and single dsi */
			lcd_dev[sel]->tcon0_ctl.bits.tcon0_if = 1;
#if defined(DSI_VERSION_40)
			...
			if (panel->lcd_tcon_mode == DISP_TCON_DUAL_DSI) {
				...
				/* mark */
				/* set tconlcd timing to auto mode */
			} else
				tcon0_dsi_clk_enable(sel, 1);
#endif
		}
...
{
	de_gamma_set_table(extend_panel->mgr_id, extend_panel->lcd_gamma_en,
		   extend_panel->is_lcd_gamma_tbl_10bit, extend_panel->lcd_gamma_tbl);
...
s32 tcon1_volume_force(u32 sel, u32 value)
{
	lcd_dev[sel]->tcon_volume_ctl.dwval = value;
	return 0;
}
