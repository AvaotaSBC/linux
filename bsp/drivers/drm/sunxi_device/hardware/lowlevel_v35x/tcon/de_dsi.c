...
s32 dsi_combo_phy_set_reg_base(u32 sel, uintptr_t base)
{
	dphy_dev[sel] = (struct __de_dsi_dphy_dev_t *) base;
	return 0;
}
	...
	if ((panel->lcd_dsi_if == LCD_DSI_IF_VIDEO_MODE)
		...
		if (panel->lcd_dsi_if == LCD_DSI_IF_BURST_MODE) {
			...
			dsi_dev[sel]->dsi_basic_ctl.bits.hbp_dis = 1;
				...
				panel->lcd_dsi_port_num == 1) {
			int overlap = 0;

			dsi_hsa = hspw / 2 * dsi_pixel_bits[format] / 8 - (4 + 4 + 2);
			dsi_hbp = hbp / 2 * dsi_pixel_bits[format] / 8 - (4 + 4 + 2);
			dsi_dev[sel]->dsi_pixel_ph.bits.wc = (panel->lcd_x / 2 + overlap) * \
				dsi_pixel_bits[panel->lcd_dsi_format] / 8;

			dsi_hfp = ((ht - hspw - hbp) / 2 - (x / 2 + overlap)) * 3 - 6 - 6;
			dsi_hblk = (ht - hspw) / 2 * dsi_pixel_bits[format] / 8 - (4 + 4 + 2);

			if (lane == 4)
				dsi_vblk = (ht / 2 - hspw / 2) * 3 - 10;  /* OK  */
			else
				dsi_vblk = 0;

			/* Switch to TCON to trigger DSI work */
			dsi_dev[sel]->dsi_basic_ctl.bits.start_mode = 1;
		} else {
			dsi_hsa =
