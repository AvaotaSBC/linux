...
int de_deband_pq_proc(u32 sel, u32 cmd, u32 subcmd, void *data)
{
	...
	if (subcmd == 16) { /* read */
		...
		para->value[12] = reg->hmin_steps.bits.hmin_steps;
		para->value[13] = reg->hmin_steps.bits.hmin_ratio;
		para->value[14] = reg->hfilt_para0.bits.kmax;
		para->value[15] = reg->hfilt_para0.bits.hsigma;
		para->value[16] = reg->rand_dither.bits.rand_num_bits;
		/*para->value[17] = reg->demo_horz.bits.demo_horz_start;
		para->value[18] = reg->demo_horz.bits.demo_horz_end;
		para->value[19] = reg->demo_vert.bits.demo_vert_start;
		para->value[20] = reg->demo_vert.bits.demo_vert_end;*/
		para->value[17] = win_per.hor_start;
		para->value[18] = win_per.hor_end;
		para->value[19] = win_per.ver_start;
		para->value[20] = win_per.ver_end;
