...
static s32 de_rtmx_init_rcq(u32 disp)
{
	...
	s32 (*get_reg_blocks[])(u32, struct de_reg_block **, u32 *) = {
		...
		de_tfbd_get_reg_blocks,
		de_crc_get_reg_blocks,
...
s32 de_rtmx_init(struct disp_bsp_init_para *para)
{
	...
	for (disp = 0; disp < disp_num; ++disp) {
		...
		de_tfbd_init(disp, ctx->de_reg_base);
		de_crc_init(disp, ctx->de_reg_base);
...
s32 de_rtmx_exit(void)
{
	...
	de_top_mem_pool_free(NULL);//fixme de_top_mem_pool_alloc is not call by de_rtmx_init
...
s32 de_rtmx_start(u32 disp)
{
	...
	de_top_set_clk_enable(DE_CLK_CORE0 + hw_disp, 1);
...
{
	s32 hw_disp = de_feat_get_hw_disp(disp);
	de_top_set_clk_enable(DE_CLK_CORE0 + hw_disp, 0);
	return 0;
}
...
s32 de_rtmx_update_reg_ahb(u32 disp)
{
	...
	for (; p_reg_blk != p_reg_blk_end; ++p_reg_blk) {
		...
		if (reg_blk->dirty) {
			aw_memcpy_toio((void *)reg_blk->reg_addr,
...
s32 de_rtmx_mgr_apply(u32 disp, struct disp_manager_data *data)
{
	...
	if (data->flag & MANAGER_ENABLE_DIRTY) {
		...
		de_top_set_de2tcon_mux(disp, data->config.hwdev_index);
		ctx->output.tcon_id = data->config.hwdev_index;
	...
	struct disp_layer_config_data **const pdata, u32 layer_num)
{
	...
	if (chn_info->px_fmt_space == DE_FORMAT_SPACE_RGB) {
		...
		case DE_RESERVED:
			...
			if ((lay_info->fb.crop.width >> 32 <= 736)
				&& (lay_info->fb.crop.height >> 32 <= 576)) {
	...
	struct de_rtmx_context *ctx, struct de_chn_info *chn_info)
{
	...
	icsc_info.width = chn_info->ovl_out_win.width;
	icsc_info.height = chn_info->ovl_out_win.height;
	...
	if (chn_info->cdc_hdl != NULL) {
		...
		ocsc_info.width = chn_info->ovl_out_win.width;
		ocsc_info.height = chn_info->ovl_out_win.height;
	...
	if (de_feat_is_support_vep_by_chn(disp, chn)) {
		de_fcm_set_csc(disp, chn, &icsc_info, &ocsc_info);
	}
	...
	de_deband_set_outinfo(disp, ctx->output.color_space,
				ctx->output.data_bits,
				ctx->output.px_fmt_space);
	...
	struct disp_layer_config_data *data, u32 layer_num)
{
	...
	for (; data != data_end; ++data) {
		...
		if (data->config.enable) {
			...
			enable++;
			if (data->config.info.fb.tfbd_en)
				tfbd_en++;
...
*/
	...
	if (chn_info->fbd_en || chn_info->atw_en || chn_info->tfbd_en) {
		de_ovl_disable(disp, chn);
		if (chn_info->tfbd_en) {
			de_tfbd_apply_lay(disp, chn, pdata, pdata_num);
			if (de_feat_is_support_fbd_by_layer(disp, chn, 0))
				de_fbd_atw_disable(disp, chn);
		} else {
	...
	de_dci_set_corlor_range(disp, chn, chn_info->color_range);
	return 0;
}
...
s32 de_rtmx_is_input_10bit(u32 disp)
{
	unsigned int i;
	unsigned int phy_chn;
	struct de_chn_info *chn_info;
	struct de_rtmx_context *ctx = de_rtmx_get_context(disp);

	for (i = 0; i < ctx->chn_num; i++) {
		phy_chn = de_feat_get_phy_chn_id(disp, i);
		chn_info = &(ctx->chn_info[i]);
		...
		switch (chn_info->px_fmt) {
		case DE_FORMAT_ARGB_2101010:
		case DE_FORMAT_ABGR_2101010:
		case DE_FORMAT_RGBA_1010102:
		case DE_FORMAT_BGRA_1010102:
		case DE_FORMAT_YUV444_I_AYUV_10BIT:
		case DE_FORMAT_YUV444_I_VUYA_10BIT:
		case DE_FORMAT_YUV422_I_YVYU_10BIT:
		case DE_FORMAT_YUV422_I_YUYV_10BIT:
