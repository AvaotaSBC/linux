...
static s32 de_wb_set_para(u32 wb)
{
	...
	case DISP_FORMAT_YUV444_I_AYUV:
		...
		reg->wb_addr_a0.dwval = (u32)out_addr[0];
		reg->wb_addr_ah.bits.ch0_h_addr = (u32)(out_addr[0] >> 32);
	...
	case DISP_FORMAT_YUV420_SP_UVUV:
		...
		reg->wb_addr_a0.dwval = (u32)out_addr[0];
		reg->wb_addr_a1.dwval = (u32)out_addr[1];
		reg->wb_addr_ah.bits.ch0_h_addr = (u32)(out_addr[0] >> 32);
		reg->wb_addr_ah.bits.ch1_h_addr = (u32)(out_addr[1] >> 32);
	...
	case DISP_FORMAT_YUV420_P:
		...
		reg->wb_addr_a0.dwval = (u32)out_addr[0];
		reg->wb_addr_a1.dwval = (u32)out_addr[1];
		reg->wb_addr_a2.dwval = (u32)out_addr[2];
		reg->wb_addr_ah.bits.ch0_h_addr = (u32)(out_addr[0] >> 32);
		reg->wb_addr_ah.bits.ch1_h_addr = (u32)(out_addr[1] >> 32);
		reg->wb_addr_ah.bits.ch2_h_addr = (u32)(out_addr[2] >> 32);
		reg->wb_pitch0.dwval = out_buf_w;
...
static s32 de_wb_set_csc_para(u32 wb)
{
	...
	icsc_info.width = ctx->output.scn_width;
	icsc_info.height = ctx->output.scn_height;
	...
	if (priv->cdc_hdl != NULL) {
		if (icsc_info.px_fmt_space == DE_FORMAT_SPACE_YUV) {
			...
			ocsc_info.width = ctx->output.scn_width;
			ocsc_info.height = ctx->output.scn_height;
		} else {
...
s32 de_wb_set_all_rcq_head_dirty(u32 wb, u32 dirty)
{
	...
	if (hd == NULL && de_feat_is_using_wb_rcq(wb) == 1) {
