...
struct de_vsu_private {
	...
	void (*set_blk_dirty)(struct de_vsu_private *priv, u32 blk_id,
			      u32 dirty);
};

static struct de_vsu_private vsu_priv[DE_NUM][MAX_CHN_NUM];

static inline struct vsu8_reg *get_vsu8_reg(struct de_vsu_private *priv)
{
	return (struct vsu8_reg *)(priv->vsu8_reg_blks[0].vir_addr);
}

static inline struct vsu10_reg *get_vsu10_reg(struct de_vsu_private *priv)
{
	return (struct vsu10_reg *)(priv->vsu10_reg_blks[0].vir_addr);
}

static inline struct vsu_ed_reg *get_vsu_ed_reg(struct de_vsu_private *priv)
{
	return (struct vsu_ed_reg *)(priv->vsu_ed_reg_blks[0].vir_addr);
}

static inline struct asu_reg *get_asu_reg(struct de_vsu_private *priv)
{
	return (struct asu_reg *)(priv->asu_reg_blks[0].vir_addr);
}

static void vsu_set_block_dirty(struct de_vsu_private *priv, u32 blk_id,
				u32 dirty)
{
	priv->reg_blks[blk_id].dirty = dirty;
}

static void vsu_set_rcq_head_dirty(struct de_vsu_private *priv, u32 blk_id,
				   u32 dirty)
{
...
s32 de_vsu_calc_lay_scale_para(u32 disp, u32 chn, enum de_format_space fm_space,
			       ...
			       struct de_scale_para *cpara)
{
	...
	if (chn_info->yuv_sampling == DE_YUV422) {
		...
		if (chn_info->snr_en) {
			...
			if (chn_info->px_fmt == DE_FORMAT_YUV420_P ||
				...
				if (priv->vsu_type == DE_SCALER_TYPE_VSU_ED ||
				    priv->vsu_type == DE_SCALER_TYPE_ASU) {
					linebuf =
					    de_feat_get_scale_linebuf_for_ed(
						disp, chn);
					if (((chn_info->ovl_out_win.width <=
					      linebuf) &&
					     (chn_info->ovl_ypara.hstep <
					      (1 << VSU_STEP_FRAC_BITWIDTH)) &&
					     (chn_info->ovl_ypara.vstep <
					      (1 << VSU_STEP_FRAC_BITWIDTH)))) {
						scale_mode = 2;
					}
				}
		...
		if (((crop32->left & 0x3) == 0x0) &&
			...
			/* odd crop_x, crop down x, and phase + 1 */
			ypara->hphase +=
			    ((crop32->left & 0x3) << VSU_PHASE_FRAC_BITWIDTH);
...
u32 de_vsu_calc_ovl_coord(u32 disp, u32 chn, u32 dst_coord, u32 scale_step)
{
	u32 half, src_coord;

	half = (1 << (VSU_STEP_FRAC_BITWIDTH - 1));
	src_coord = (dst_coord * scale_step + half) >> VSU_STEP_FRAC_BITWIDTH;
	DE_INF("half_shift_coord=<%x,%d,%d,%d>\n", half, VSU_STEP_FRAC_BITWIDTH,
	       dst_coord, src_coord);

	return src_coord;
}

s32 de_vsu_calc_ovl_scale_para(u32 layer_num, struct de_scale_para *ypara,
			       struct de_scale_para *cpara,
...
u32 de_vsu_fix_tiny_size(u32 disp, u32 chn, struct de_rect_s *in_win,
...
u32 de_vsu_fix_big_size(u32 disp, u32 chn, struct de_rect_s *in_win,
...
s32 de_vsu_calc_scale_para(u32 fix_size_result, enum de_format_space fmt_space,
			   enum de_yuv_sampling yuv_sampling,
...
static s32 de_vsu8_set_para(u32 disp, u32 chn, struct de_chn_info *chn_info)
{
	struct de_vsu_private *priv = &(vsu_priv[disp][chn]);
	...
	reg->out_size.bits.width =
	    chn_info->scn_win.width ? (chn_info->scn_win.width - 1) : 0;
	reg->out_size.bits.height =
	    chn_info->scn_win.height ? (chn_info->scn_win.height - 1) : 0;

	reg->glb_alpha.dwval = chn_info->glb_alpha;
	priv->set_blk_dirty(priv, VSU8_REG_BLK_ATTR, 1);

	reg->y_in_size.bits.width =
	    chn_info->ovl_out_win.width ? (chn_info->ovl_out_win.width - 1) : 0;
	reg->y_in_size.bits.height = chn_info->ovl_out_win.height
					 ? (chn_info->ovl_out_win.height - 1)
					 : 0;
	...
	reg->c_in_size.bits.width =
	    chn_info->c_win.width ? (chn_info->c_win.width - 1) : 0;
	reg->c_in_size.bits.height =
	    chn_info->c_win.height ? (chn_info->c_win.height - 1) : 0;
	...
	aw_memcpy_toio(reg->y_hori_coeff, lan2coefftab32 + pt_coef,
	       sizeof(u32) * VSU_PHASE_NUM);
	pt_coef = de_vsu_calc_fir_coef(chn_info->ovl_ypara.vstep);
	aw_memcpy_toio(reg->y_vert_coeff, lan2coefftab32 + pt_coef,
	       sizeof(u32) * VSU_PHASE_NUM);

	/* ch1/2 */
	if (chn_info->px_fmt_space == DE_FORMAT_SPACE_RGB) {
		pt_coef = de_vsu_calc_fir_coef(chn_info->ovl_cpara.hstep);
		aw_memcpy_toio(reg->c_hori_coeff, lan2coefftab32 + pt_coef,
		       sizeof(u32) * VSU_PHASE_NUM);
	} else {
		pt_coef = de_vsu_calc_fir_coef(chn_info->ovl_cpara.hstep);
		aw_memcpy_toio(reg->c_hori_coeff, bicubic4coefftab32 + pt_coef,
		       sizeof(u32) * VSU_PHASE_NUM);
	}
...
static s32 de_vsu10_set_para(u32 disp, u32 chn, struct de_chn_info *chn_info)
{
	struct de_vsu_private *priv = &(vsu_priv[disp][chn]);
	...
	/* ch0 */
	pt_coef = de_vsu_calc_fir_coef(chn_info->ovl_ypara.hstep);
	aw_memcpy_toio(reg->y_hori_coeff0, lan3coefftab32_left + pt_coef,
	       sizeof(u32) * VSU_PHASE_NUM);
	aw_memcpy_toio(reg->y_hori_coeff1, lan3coefftab32_right + pt_coef,
	       sizeof(u32) * VSU_PHASE_NUM);
	pt_coef = de_vsu_calc_fir_coef(chn_info->ovl_ypara.vstep);
	aw_memcpy_toio(reg->y_vert_coeff, lan2coefftab32 + pt_coef,
	       sizeof(u32) * VSU_PHASE_NUM);
	...
	if (chn_info->px_fmt_space == DE_FORMAT_SPACE_RGB) {
		pt_coef = de_vsu_calc_fir_coef(chn_info->ovl_cpara.hstep);
		aw_memcpy_toio(reg->c_hori_coeff0, lan3coefftab32_left + pt_coef,
		       sizeof(u32) * VSU_PHASE_NUM);
		aw_memcpy_toio(reg->c_hori_coeff1, lan3coefftab32_right + pt_coef,
		       sizeof(u32) * VSU_PHASE_NUM);
		pt_coef = de_vsu_calc_fir_coef(chn_info->ovl_cpara.vstep);
		aw_memcpy_toio(reg->c_vert_coeff, lan2coefftab32 + pt_coef,
		       sizeof(u32) * VSU_PHASE_NUM);
	} else {
		pt_coef = de_vsu_calc_fir_coef(chn_info->ovl_cpara.hstep);
		aw_memcpy_toio(reg->c_hori_coeff0, bicubic8coefftab32_left + pt_coef,
		       sizeof(u32) * VSU_PHASE_NUM);
		aw_memcpy_toio(reg->c_hori_coeff1, bicubic8coefftab32_right + pt_coef,
		       sizeof(u32) * VSU_PHASE_NUM);
		pt_coef = de_vsu_calc_fir_coef(chn_info->ovl_cpara.vstep);
		aw_memcpy_toio(reg->c_vert_coeff, bicubic4coefftab32 + pt_coef,
		       sizeof(u32) * VSU_PHASE_NUM);
	}
...
static s32 de_vsu_ed_set_para(u32 disp, u32 chn, struct de_chn_info *chn_info)
{
	struct de_vsu_private *priv = &(vsu_priv[disp][chn]);
...
static s32 de_asu_set_para(u32 disp, u32 chn, struct de_chn_info *chn_info)
{
	struct de_vsu_private *priv = &(vsu_priv[disp][chn]);
	...
	reg->g_alpha.dwval = chn_info->glb_alpha;
	priv->set_blk_dirty(priv, ASU_REG_BLK_CTL, 1);
	...
	priv->set_blk_dirty(priv, ASU_REG_BLK_ATTR, 1);

	reg->y_size.bits.y_width =
	    chn_info->ovl_out_win.width ? (chn_info->ovl_out_win.width - 1) : 0;
	reg->y_size.bits.y_height = chn_info->ovl_out_win.height
					? (chn_info->ovl_out_win.height - 1)
					: 0;
	reg->y_hstep.dwval = chn_info->ovl_ypara.hstep
			     << VSU_STEP_VALID_START_BIT;
	reg->y_vstep.dwval = chn_info->ovl_ypara.vstep
			     << VSU_STEP_VALID_START_BIT;
	reg->y_hphase.dwval = chn_info->ovl_ypara.hphase
	...
	reg->y_vphase0.dwval = chn_info->ovl_ypara.vphase
			       << VSU_PHASE_VALID_START_BIT;
	priv->set_blk_dirty(priv, ASU_REG_BLK_YPARA, 1);
	...
	priv->set_blk_dirty(priv, ASU_REG_BLK_CPARA, 1);

	/* fir coefficient */
	/* ch0 */
	pt_coef = de_vsu_calc_fir_coef(chn_info->ovl_ypara.hstep);
	aw_memcpy_toio(reg->y_hcoef0, lan3coefftab32_left + pt_coef,
	       sizeof(u32) * VSU_PHASE_NUM);
	aw_memcpy_toio(reg->y_hcoef1, lan3coefftab32_right + pt_coef,
	       sizeof(u32) * VSU_PHASE_NUM);
	pt_coef = de_vsu_calc_fir_coef(chn_info->ovl_ypara.vstep);
	aw_memcpy_toio(reg->y_vcoef, lan2coefftab32 + pt_coef,
	       sizeof(u32) * VSU_PHASE_NUM);

	/* ch1/2 */
	if (chn_info->px_fmt_space == DE_FORMAT_SPACE_RGB) {
		pt_coef = de_vsu_calc_fir_coef(chn_info->ovl_cpara.hstep);
		aw_memcpy_toio(reg->c_hcoef0, lan3coefftab32_left + pt_coef,
		       sizeof(u32) * VSU_PHASE_NUM);
		aw_memcpy_toio(reg->c_hcoef1, lan3coefftab32_right + pt_coef,
		       sizeof(u32) * VSU_PHASE_NUM);
		pt_coef = de_vsu_calc_fir_coef(chn_info->ovl_cpara.vstep);
		aw_memcpy_toio(reg->c_vcoef, lan2coefftab32 + pt_coef,
		       sizeof(u32) * VSU_PHASE_NUM);
	} else {
		pt_coef = de_vsu_calc_fir_coef(chn_info->ovl_cpara.hstep);
		aw_memcpy_toio(reg->c_hcoef0, bicubic8coefftab32_left + pt_coef,
		       sizeof(u32) * VSU_PHASE_NUM);
		aw_memcpy_toio(reg->c_hcoef1, bicubic8coefftab32_right + pt_coef,
		       sizeof(u32) * VSU_PHASE_NUM);
		pt_coef = de_vsu_calc_fir_coef(chn_info->ovl_cpara.vstep);
		aw_memcpy_toio(reg->c_vcoef, bicubic4coefftab32 + pt_coef,
		       sizeof(u32) * VSU_PHASE_NUM);
	}
	priv->set_blk_dirty(priv, ASU_REG_BLK_COEFF0, 1);
	priv->set_blk_dirty(priv, ASU_REG_BLK_COEFF1, 1);
	priv->set_blk_dirty(priv, ASU_REG_BLK_COEFF2, 1);
	priv->set_blk_dirty(priv, ASU_REG_BLK_COEFF3, 1);
	priv->set_blk_dirty(priv, ASU_REG_BLK_COEFF4, 1);
	priv->set_blk_dirty(priv, ASU_REG_BLK_COEFF5, 1);

	return 0;
}

s32 de_vsu_set_para(u32 disp, u32 chn, struct de_chn_info *chn_info)
{
	struct de_vsu_private *priv = &(vsu_priv[disp][chn]);
	...
	} else if (priv->vsu_type == DE_SCALER_TYPE_ASU) {
		return de_asu_set_para(disp, chn, chn_info);
	}
...
s32 de_vsu_set_sharp_para(u32 disp, u32 chn, u32 fmt, u32 dev_type,
...
s32 de_vsu_init_peaking_para(u32 disp, u32 chn)
{
	...
	if (priv->vsu_type == DE_SCALER_TYPE_ASU) {
		struct asu_reg *reg = get_asu_reg(priv);

		reg->peaking_en.dwval = 0x80010001;
		reg->peaking_stren.dwval = 0x40004;
		reg->peaking_limit.dwval = 0xc803ff;
		reg->peaking_para.dwval = 0x20a0200;
		priv->set_blk_dirty(priv, ASU_REG_BLK_PEAKING, 1);
	}
	return 0;
}

s32 de_vsu_set_peaking_para(u32 disp, u32 chn, u32 fmt, u32 dev_type,
			    struct de_asu_peaking_config *para, u32 bypass)
{
	struct de_vsu_private *priv = &(vsu_priv[disp][chn]);
	u8 level;
	u8 en;
	/*level 0- 5 support only*/
	static u8 MAX_PEAKING = 5;

	if (priv->vsu_type == DE_SCALER_TYPE_ASU) {
		struct asu_reg *reg = get_asu_reg(priv);
		if ((para->level > 0) && (fmt == 0) && (bypass == 0)) {
			en = 1;
		} else {
			en = 0;
		}
		if (en) {
			reg->peaking_en.dwval = 0x80010001;
			level = para->level % MAX_PEAKING;
			level = 1 << level;
			reg->peaking_stren.bits.peaking_strength = level;
			reg->peaking_stren.bits.cpeaking_strength = level;
		} else {
			reg->peaking_en.dwval = 0x80000000;
		}
		priv->set_blk_dirty(priv, ASU_REG_BLK_PEAKING, 1);
	}
	return 0;
...
s32 de_vsu_set_peaking_window(u32 disp, u32 chn, u32 win_enable,
			      struct de_rect_o window)
{
	...
	if (priv->vsu_type == DE_SCALER_TYPE_ASU) {
		struct asu_reg *reg = get_asu_reg(priv);
		reg->ctl.bits.win_en = win_enable;
		if (win_enable) {
			reg->peaking_en.dwval = 0x80010001;
			reg->peaking_stren.bits.peaking_strength = 3;
			reg->peaking_stren.bits.cpeaking_strength = 3;
			reg->demo_horz.bits.win_left = window.x;
			reg->demo_horz.bits.win_right = window.x + window.w - 1;
			reg->demo_vert.bits.win_top = window.y;
			reg->demo_vert.bits.win_bot = window.y + window.h - 1;
		} else {
			reg->peaking_en.dwval = 0x80010001;
		}
		priv->set_blk_dirty(priv, ASU_REG_BLK_ATTR, 1);
...
s32 de_vsu_get_reg_blocks(u32 disp, struct de_reg_block **blks, u32 *blk_num)
{
	u32 chn_num, chn;
	u32 total = 0;
