...
static s32 smbl_init(u32 disp, uintptr_t reg_base)
{
	...
	base = reg_base + DE_DISP_OFFSET(disp) + DISP_DRC_OFFSET;
...
static s32 de_smbl_get_hist(u32 disp, u32 *cnt)
{
	...
	if (g_smbl_status[disp]->ahb_update_en)
		aw_memcpy_fromio((u8 *)cnt, reg_addr, sizeof(u32) * IEP_LH_INTERVAL_NUM);
...
static s32 de_smbl_set_lut(u32 disp, u16 *lut)
{
	...
	aw_memcpy_toio((void *)reg->drclgcoff, (void *)lut, sizeof(reg->drclgcoff));
...
s32 de_smbl_tasklet(u32 disp)
{
	...
	if (g_smbl_status[disp]->isenable
		&& g_smbl_status[disp]->ahb_update_en
...
static s32 de_smbl_set_para(u32 disp, u32 width, u32 height)
{
	...
	/* out_csc coeff */
	aw_memcpy_toio((void *)(priv->reg_blks[SMBL_CSC_REG_BLK].reg_addr),
	       (u8 *)csc_bypass_coeff, sizeof(u32) * 12);

	/* filter coeff */
	aw_memcpy_toio((void *)(priv->reg_blks[SMBL_FILTER_REG_BLK].reg_addr),
			...
			struct disp_rect window)
{
	...
	if (win_enable && window.width && window.height) {
...
s32 de_smbl_enable_ahb_read(u32 disp, bool en)
{
	DE_WRN("de_smbl_enable_ahb_read=%d\n", en);
	g_smbl_status[disp]->ahb_update_en = en;
	return 0;
}
#else
