...
struct de_fbd_atw_private {
	...
	void (*set_blk_dirty)(struct de_fbd_atw_private *priv, u32 blk_id,
			      u32 dirty);
};
...
static void fbd_atw_set_block_dirty(struct de_fbd_atw_private *priv, u32 blk_id,
				    u32 dirty)
{
	priv->reg_blks[blk_id].dirty = dirty;
}

static void fbd_atw_set_rcq_head_dirty(struct de_fbd_atw_private *priv,
...
static s32 de_fbd_get_info(struct disp_afbc_info *afbc, struct de_fbd_info *info)
{
	u32 inputbits;
			      ...
			      u32 layer_num)
{
	...
	de_fbd_get_info(&lay_info->fb.afbc_info, &fbd_info);
	...
	case DISP_FORMAT_YUV420_P_10BIT:
		dwval = 0x3210;
		break;
	default:
		dwval = 0x3210;
		DE_WRN("unsupported format=%x\n", lay_info->fb.format);
	}
	...
	width  = lay_info->fb.afbc_info.block_size[0];
	height = lay_info->fb.afbc_info.block_size[1];
	dwval = (width & 0x3FF) | ((height & 0x3FF) << 16);
	reg->fbd_blk_size.dwval = dwval;

	left = lay_info->fb.afbc_info.image_crop[0];
	top  = lay_info->fb.afbc_info.image_crop[1];
			      ...
			      u32 layer_num)
{
	...
	struct disp_layer_info_inner *lay_info = &(pdata[0]->config.info);
	u32 dwval;
	u32 width, height;
	u32 b_col, b_row;
	...
	dwval =
	    (lay_info->atw.b_col & 0xFF) | ((lay_info->atw.b_row & 0xFF) << 8);
...
s32 de_fbd_atw_get_reg_blocks(u32 disp, struct de_reg_block **blks,
			      u32 *blk_num)
{
	u32 chn_num, chn;
	u32 total = 0;
