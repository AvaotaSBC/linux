...
/*
	...
	pitch[0] = lay_cfg->info.fb.pitch[0];
	pitch[1] = lay_cfg->info.fb.pitch[1];
	pitch[2] = lay_cfg->info.fb.pitch[2];
...
{
	if (chn_info->scale_en && (chn_info->ovl_out_win.width != 0)
	...
	if (chn_info->scale_en && (chn_info->ovl_out_win.height != 0)
