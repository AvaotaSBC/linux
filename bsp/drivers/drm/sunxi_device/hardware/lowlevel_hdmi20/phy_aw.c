...
static struct aw_phy_params aw_phy[] = {
...
static struct aw_phy_params *_aw_phy_get_configs(u32 pClk, dw_color_depth_t color,
...
static void _aw_phy_encoding_adapt(dw_color_format_t format)
{
	...
	if (format == DW_COLOR_FORMAT_YCC420)
...
static void _aw_phy_set_clk(void)
{
	...
	/* aw_phy_addr->phy_ctl2.bits.reg_resdi = 0x18; */
	/* low power voltage 1.08V, default 3 */
	/* aw_phy_addr->phy_ctl4.bits.reg_slv = 3; */
...
static int _aw_phy_enable(void)
{
	...
	if ((i == AW_PHY_TIMEOUT) && !status) {
		hdmi_err("phy_rcalend2d_status Timeout !\n");
		return -1;
	}
	...
	if ((i == AW_PHY_TIMEOUT) && !status) {
		hdmi_err("pll_lock_status Timeout! status = 0x%x\n", status);
		return -1;
	}
	...
	if ((i == AW_PHY_TIMEOUT) && !status) {
		hdmi_err("tx_ready_status Timeout ! status = 0x%x\n", status);
		return -1;
	}
...
static int _aw_phy_config(struct aw_phy_params *config)
{
	int ret = 0;
	/* colse PHY and mPLL */
	_aw_phy_reset();
...
int aw_phy_config(void)
{
	...
	config = _aw_phy_get_configs(pixel_clk, bits, pixel);

	if (config == NULL) {
		hdmi_err("aw phy table unsupport clock %dhz, color depth %d, pixel repetition %d\n",
...
int aw_phy_write(u8 addr, void *data)
{
	...
	*((u8 *)((void *)aw_phy_addr + addr)) = *value;
	return 0;
}
...
int aw_phy_read(u8 addr, void *data)
{
	...
	*value = *((u8 *)((void *)aw_phy_addr + addr));
	return 0;
}
