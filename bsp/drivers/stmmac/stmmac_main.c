...
static u32 stmmac_tx_avail(struct stmmac_priv *priv, u32 queue)
{
 ...
 */
static u32 stmmac_rx_dirty(struct stmmac_priv *priv, u32 queue)
{
					...
					struct dma_desc *p, struct sk_buff *skb)
{
	...
	struct skb_shared_hwtstamps *shwtst = &shhwtstamp;
	bool found = false;
	u64 ns = 0;
	...
	if (found) {
		...
		memset(&shhwtstamp, 0, sizeof(*shwtst));
					...
					struct dma_desc *np, struct sk_buff *skb)
{
	...
	if (stmmac_get_rx_timestamp_status(priv, p, np, priv->adv_ts)) {
		...
		memset(shhwtstamp, 0, sizeof(*shhwtstamp));
		shhwtstamp->hwtstamp = ns_to_ktime(ns);
	} else {
 ...
 */
static void stmmac_rx_refill(struct stmmac_priv *priv, u32 queue)
{
	struct stmmac_rx_queue *rx_q = &priv->rx_queue[queue];
			...
			struct stmmac_resources *res)
{
	...
	ndev = devm_alloc_etherdev_mqs(device, sizeof(*priv),
