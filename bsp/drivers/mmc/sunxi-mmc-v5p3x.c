...
#define SUNXI_RETRY_ARRAY_NUM				7
...
clk_set_retry:
	...
	if (ios->clock > 400 * 1000) {
		...
		if (sunxi_mmc_ddr_timing(ios->timing))
			SM_INFO(mmc_dev(host->mmc), "Warning: is 400KHz DDR mode");
		rval &= ~(1 << 7);
		sunxi_r_op(host, mmc_writel(host, REG_DRV_DL, rval));
	}
#else
	void __iomem *card2_int_sg_en = ioremap(0x430000b0, 0x4);

	u32 reg_val = readl(card2_int_sg_en);
	if (ios->clock > 400 * 1000) {
		reg_val |= (1 << 31);
		writel(reg_val, card2_int_sg_en);
				       ...
				       u32 errno, void *other)
{
	...
	*/
	const s32 sunxi_phase[SUNXI_RETRY_ARRAY_NUM][2] = { {-1, -1},
		{1, 1}, {0, 0}, {1, 0}, {0, 1}, {1, 2}, {0, 2} };

	if (rcnt < (SUNXI_RETRY_CNT_PER_PHA_V5P3X * SUNXI_RETRY_ARRAY_NUM)) {
	...
	SM_INFO(mmc_dev(host->mmc), "sunxi v5p3x retry give up\n");
	return -1;
}
