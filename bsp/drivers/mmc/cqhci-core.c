...
{
	struct mmc_data *data = mrq->data;
	int i, sg_count, len, size = 0;
	bool end = false, out_4g = false;
	bool dma64 = cq_host->dma64;
	dma_addr_t addr;
	u8 *desc;
	struct scatterlist *sg, *bksg;
	struct page *page;
	int nr_page;
	void *buf;
	...
	for_each_sg(data->sg, sg, sg_count, i) {
		...
		if (cq_host->cqhci_get_soc_ver < 2) {
			size += len;

			/*check addr */
			if ((addr + len) & 0xffffffff00000000) {
				pr_err("****addr:%llu, len:%d\n", addr, len);
				out_4g = true;
			}
		}
	}
	...
	if (out_4g) {
		nr_page = PAGE_ALIGN(size) / PAGE_SIZE;
		sg = bksg = cq_host->sg_slot[tag].sgtable.sgl;
		bksg->length = 0;
		page = cq_host->pages_slot[tag].pages;
		buf = page_to_virt(cq_host->pages_slot[tag].pages);
		for (i = 0; i < nr_page; i++) {
			sg_set_page(sg, page++, PAGE_SIZE, 0);
			sg_unmark_end(sg);

			if (i == (nr_page - 1)) {
				sg->length = (size - 1) % PAGE_SIZE + 1;
				sg_mark_end(sg);
			}
		...
		cq_host->sg_slot[tag].sg_len = nr_page;
		cq_host->pages_slot[tag].size = size;

		if (data->flags & MMC_DATA_WRITE) {
			sg_copy_to_buffer(data->sg, data->sg_len, buf, size);
		}
		sg_count = dma_map_sg(mmc_dev(mrq->host), bksg, nr_page,
	...
	if (!out_4g) {
		...
		for_each_sg(bksg, sg, sg_count, i) {
			addr = sg_dma_address(sg);
			len = sg_dma_len(sg);
...
static void cqhci_post_req(struct mmc_host *host, struct mmc_request *mrq)
{
	...
	if (data) {
		...
		if (cq_host->pages_slot[tag].size) {
			bksg = cq_host->sg_slot[tag].sgtable.sgl;
			nr_page = cq_host->sg_slot[tag].sg_len;
			buf = page_to_virt(cq_host->pages_slot[tag].pages);
			dma_unmap_sg(mmc_dev(host), bksg, nr_page,
				     (data->flags & MMC_DATA_READ) ?
				     DMA_FROM_DEVICE : DMA_TO_DEVICE);
			if (data->flags & MMC_DATA_READ) {
				sg_copy_from_buffer(data->sg, data->sg_len, buf, cq_host->pages_slot[tag].size);
			}
			cq_host->pages_slot[tag].size = 0;
		}
	}
}
#else
...
#if ((LINUX_VERSION_CODE >= KERNEL_VERSION(5, 15, 137) && defined (CONFIG_AW_KERNEL_AOSP))) || \
	((LINUX_VERSION_CODE >= KERNEL_VERSION(5, 16, 0) && defined (CONFIG_AW_KERNEL_ORIGIN)))
...
#if defined(CONFIG_ARCH_SUN55IW3)
void cqhci_deinit(struct cqhci_host *cq_host)
{
	int i;
	struct mmc_host *mmc = cq_host->mmc;

	if (totalram_pages() > (3 * 1024 * 1024 / 4)) {
		for (i = 0; i < 32; i++) {
			if (cq_host->pages_slot[i].pages) {
				free_pages((unsigned long)(cq_host->pages_slot[i].pages), get_order(mmc->max_req_size));
				cq_host->pages_slot[i].pages = NULL;
				sg_free_table(&(cq_host->sg_slot[i].sgtable));
			}
		}
	}
}
EXPORT_SYMBOL(cqhci_deinit);
	      ...
	      bool dma64)
{
	...
	cq_host->cqhci_get_soc_ver = sunxi_get_soc_ver();
	...
	if (cq_host->cqhci_get_soc_ver < 2) {
		for (i = 0; i < 32; i++) {
			cq_host->pages_slot[i].pages = NULL;
			cq_host->pages_slot[i].size = 0;

			/*Only more than 3g of memory can be copied.
			 * In order to prevent cqhci from accessing
			 * more than 4G of space */
			if (totalram_pages() > (3 * 1024 * 1024 / 4)) {
				if (mmc->max_req_size > 1024 * 1024)
					mmc->max_req_size = 1024 * 1024;
				cq_host->pages_slot[i].pages = alloc_pages(GFP_DMA32 | GFP_KERNEL, get_order(mmc->max_req_size));
				if (!cq_host->pages_slot[i].pages) {
					pr_err(" %s: unable to alloc page, num:%d\n",
						 __func__, get_order(mmc->max_req_size));
					err = -ENOMEM;
					goto cqhci_deinit;
				}
				ret = sg_alloc_table(&(cq_host->sg_slot[i].sgtable), PAGE_SIZE, GFP_KERNEL);
				if (ret) {
					free_pages((unsigned long)(cq_host->pages_slot[i].pages), get_order(mmc->max_req_size));
					cq_host->pages_slot[i].pages = NULL;
					pr_err(" %s: unable to alloc sg table : i:%d\n",
						 __func__, i);
					err = -ENOMEM;
					goto cqhci_deinit;
				}
