				  ...
				  const char *buf, size_t count)
{
	...
	if (insert)
		...
		caps2_old = mmc->caps2;
		if (mmc->card == NULL) {
			SM_INFO(dev, "the device has been removed\n");
...
static int sunxi_mmc_res_start_addr(const char * const res_str,
				...
				struct device_attribute *attr, char *buf)
{
	...
	if (value < 0) {
		SM_ERR(dev, "gpio get value fail\n");
	} else {
		SM_INFO(dev, "%d\n", value);
	}
		...
		if (value < 0) {
			...
			if (value == 0) {
				if (host->ctl_spec_cap & CARD_PWR_GPIO_HIGH_ACTIVE) {
					SM_INFO(dev, "power off\n");
					return sprintf(buf, "gpio value = %d, power off\n", value);
				} else {
					SM_INFO(dev, "power on\n");
				...
				SM_ERR(dev, "gpio get value is illegal, value = %d\n", value);
			}
		}
	} else {
		SM_ERR(dev, "card_pwr_gpio get fail\n");
	}
		...
		const char *buf, size_t count)
{
	...
	if (!gpio_is_valid(host->card_pwr_gpio)) {
		SM_ERR(dev, "error: card_pwr_gpio is invalid\n");
		return -1;
	}

	if (host->card_pwr_mode != CARD_PWR_GPIO_MANUAL) {
		SM_ERR(dev, "error: need manual mode\n");
		return -1;
	}
	...
	SM_INFO(dev, "get gpio value %lld\n", value);
	if (value == 0) {
				...
				struct device_attribute *attr, char *buf)
{
	...
	if (host->card_pwr_mode == CARD_PWR_GPIO_AUTO) {
		SM_INFO(dev, "auto mode\n");
	} else if (host->card_pwr_mode == CARD_PWR_GPIO_MANUAL) {
		SM_INFO(dev, "manual mode\n");
	} else {
		SM_ERR(dev, "unknown mode\n");
	}
		...
		const char *buf, size_t count)
{
	...
	sscanf(buf, "%lld", &value);
	SM_INFO(dev, "get mode value %lld\n", value);
	mmc_claim_host(mmc);
	...
	if (value == CARD_PWR_GPIO_AUTO) {
		...
		SM_ERR(dev, "set mode err: unknown mode\n");
	}
...
sunxi_mmc_show_perf(struct device *dev, struct device_attribute *attr, char *buf)
{
	...
	return sprintf(buf, "Write performance at host driver Level:"
...
extern ssize_t sunxi_mmc_panic_rtest(struct device *dev, struct device_attribute *attr, char *buf);
extern ssize_t sunxi_mmc_pancic_wrtest(struct device *dev, struct device_attribute *attr, const char *buf, size_t count);
...
sunxi_mmc_show_filter_sector(struct device *dev, struct device_attribute *attr, char *buf)
{
	...
	return sprintf(buf, "filter speed %d\n", host->filter_sector);
}
...
sunxi_mmc_show_dbglevel(struct device *dev, struct device_attribute *attr, char *buf)
{
	...
	return sprintf(buf, "Current debuglevel %d\n", host->debuglevel);
}
...
static ssize_t
sunxi_mmc_show_timing(struct device *dev, struct device_attribute *attr, char *buf)
{
	struct platform_device *pdev = to_platform_device(dev);
	struct mmc_host	*mmc = platform_get_drvdata(pdev);
	...
	return sprintf(buf, "timing : %s\n", timing[mmc->ios.timing]);
}

static ssize_t
sunxi_mmc_set_timing(struct device *dev, struct device_attribute *attr,
		const char *buf, size_t count)
{
	struct platform_device *pdev = to_platform_device(dev);
	struct mmc_host	*mmc = platform_get_drvdata(pdev);
...
int mmc_create_sys_fs(struct sunxi_mmc_host *host, struct platform_device *pdev)
{
	...
	host->host_timing.show = sunxi_mmc_show_timing;
	host->host_timing.store = sunxi_mmc_set_timing;
	sysfs_attr_init(&(host->host_timing.attr));
	host->host_timing.attr.name = "sunxi_mmc_timing";
	host->host_timing.attr.mode =  S_IRUGO | S_IWUSR;
	ret = device_create_file(&pdev->dev, &host->host_timing);
	if (ret)
		return ret;
		       ...
		       struct platform_device *pdev)
{
	...
	device_remove_file(&pdev->dev, &host->host_timing);
