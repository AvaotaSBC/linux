...
#define DISP2_DEBUG_LEVEL 2

#if DISP2_DEBUG_LEVEL == 3
#define __dev_err(dev, format, args...) sunxi_err(dev, "[DE]: " format, ## args)
#define __err(format, args...) sunxi_err(NULL, "[DE]: " format, ## args)
#define __wrn(format, args...)
#define __inf(format, args...)
#define __debug(format, args...)
#elif DISP2_DEBUG_LEVEL == 2
#define __dev_err(dev, format, args...) sunxi_err(dev, "[DE]: " format, ## args)
#define __err(format, args...) sunxi_err(NULL, "[DE]: " format, ## args)
#define __wrn(format, args...) sunxi_warn(NULL, "[DE]: " format, ## args)
#define __inf(format, args...)
#define __debug(format, args...)
#elif DISP2_DEBUG_LEVEL == 1
#define __dev_err(dev, format, args...) sunxi_err(dev, "[DE]: " format, ## args)
#define __err(format, args...) sunxi_err(NULL, "[DE]: " format, ## args)
#define __wrn(format, args...) sunxi_warn(NULL, "[DE]: " format, ## args)
#define __inf(format, args...) sunxi_info(NULL, "[DE]: " format, ## args)
#define __debug(format, args...)
#elif DISP2_DEBUG_LEVEL == 0
#define __dev_err(dev, format, args...) sunxi_err(dev, "[DE]: " format, ## args)
#define __err(format, args...) sunxi_err(NULL, "[DE]: " format, ## args)
#define __wrn(format, args...) sunxi_warn(NULL, "[DE]: " format, ## args)
#define __inf(format, args...) sunxi_info(NULL, "[DE]: " format, ## args)
#define __debug(format, args...) sunxi_debug(NULL, "[DE]: " format, ## args)
#else
#define __dev_err(dev, format, args...)
#define __err(format, args...)
#define __wrn(format, args...)
#define __inf(format, args...)
#define __debug(format, args...)
#endif
...
#if defined(__LINUX_PLAT__)
#define DE_DEV_ERR __dev_err
#define DE_ERR __err
#define DE_WARN __wrn
#define DE_INFO __inf
#define DE_DEBUG __debug
...
#if IS_ENABLED(CONFIG_AW_FPGA_S4) \
...
enum disp_csc_hw_type {
	CSC_TYPE_NONE = 0,
	CSC_TYPE_CHN,
	CSC_TYPE_DISP,
};

enum disp_csc_dirty_flags {
	CSC_DIRTY_NONE = 0x0,
	CSC_DIRTY_CS_CONVERT = 0x1 << 0,
	CSC_DIRTY_ENHANCE = 0x1 << 1,
	CSC_DIRTY_COLOR_MATRIX = 0x1 << 2,
	CSC_DIRTY_ALL = CSC_DIRTY_CS_CONVERT | CSC_DIRTY_ENHANCE |
					CSC_DIRTY_COLOR_MATRIX,
};
...
struct disp_csc_config {
	...
	u32 chn;
	u32 csc_id;
	enum disp_csc_hw_type hw_type;
	enum disp_csc_dirty_flags flags;
...
typedef enum {
	DISP_LCD_DSI_DATA_EOTP = 1,
} disp_lcd_dsi_data_mode;
...
struct disp_panel_convert_info {
	bool is_enabled;
...
struct disp_panel_para {
	...
	disp_lcd_dsi_data_mode   lcd_dsi_data_mode;
	...
	struct disp_panel_convert_info convert_info;
};

struct disp_panel_para_mapping {
...
struct disp_health_info {
	...
	unsigned long long rcq_irq_cnt;
	unsigned long long rcq_update_req_cnt;
	unsigned long long rcq_update_req_irq;
	unsigned long long rcq_finish_irq;
	unsigned int vsync_cnt;
	unsigned int vsync_skip_cnt;
};

struct amp_preview_init_para {
	bool amp_preview[DISP_SCREEN_NUM];
	unsigned long premap_addr;
	int src_width;
	int src_height;
	bool aux_line_en;
	int screen_x;
...
struct disp_bsp_init_para {
...
#if IS_ENABLED(CONFIG_AW_DRM) || defined(SUPPORT_VGA)
	...
	struct disp_bootloader_info boot_info[DISP_DEVICE_NUM];
	struct disp_feat_init feat_init;
	struct amp_preview_init_para preview;
};
...
struct disp_lcd_panel_fun {
	...
	int (*get_panel_para_mapping)(const struct disp_panel_para_mapping **);
};
...
struct disp_device {
	...
	s32 (*deliver_dev) (struct disp_device *dispdev, struct device *dev);
	s32 (*drv_init) (struct disp_device *dispdev, int dispdev_id);
	s32 (*drv_deinit) (struct disp_device *dispdev, int dispdev_id);
	...
	s32 (*set_color_temperature)(struct disp_device *dispdev,
	...
	s32 (*set_close_func)(struct disp_device *lcd, LCD_FUNC func,
			      u32 delay);
	int (*gpio_get_value)(struct disp_device *dispdev,
...
struct disp_manager {
	...
	s32 (*get_layer_config2)(struct disp_manager *mgr,
				 struct disp_layer_config2 *config,
				 unsigned int layer_num);
	s32 (*force_set_layer_config)(struct disp_manager *mgr,
				      struct disp_layer_config *config,
				      unsigned int layer_num);
	s32 (*force_set_layer_config2)(struct disp_manager *mgr,
				      struct disp_layer_config2 *config,
				      unsigned int layer_num);
	...
	s32 (*set_color_matrix)(struct disp_manager *mgr, long long *matrix);
	s32 (*set_palette)(struct disp_manager *mgr,
			   struct disp_palette_config *config);
	...
	bool (*is_fb_replaced)(struct disp_manager *mgr);
	...
	s32 (*handle_iommu_error)(struct disp_manager *mgr);
};
...
struct disp_enhance {
	...
	s32 (*pq_update)(struct disp_enhance *enhance, u32 *value);
	s32 (*pq_apply)(struct disp_enhance *enhance);
};
