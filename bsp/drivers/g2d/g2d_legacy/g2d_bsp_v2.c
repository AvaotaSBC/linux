...
static __u32 srcbuffer_size;
static __u32 dstbuffer_size;

void g2d_set_src_image_buf_size(__u32 src_image_buf_size)
{
	srcbuffer_size = src_image_buf_size;
}

void g2d_set_dst_image_buf_size(__u32 dst_image_buf_size)
{
	dstbuffer_size = dst_image_buf_size;
}
			   ...
			   enum g2d_scan_order scan_order)
{
	...
	if ((para->flag == G2D_BLT_NONE) ||
		...
		if (para->flag & 0xfe0) {
			G2D_WARN("Wrong! mixer and rot cant use at same time\n");
			return -1;
		}
...
__s32 g2d_bsp_bitblt(g2d_image_enh *src, g2d_image_enh *dst, __u32 flag)
{
	...
	if (dst == NULL) {
		G2D_WARN("dst image is NULL\n");
		return -1;
	}
	if (src == NULL) {
		G2D_WARN("src image is NULL\n");
		return -2;
	}
	G2D_INFO_MSG("BITBLT_flag:  0x%x\n", flag);
