		...
		struct wireless_dev *wdev, const void  *data, int len)
{
	...
	if (!(supported_features & DBG_MEMORY_DUMP_SUPPORTED)) {
		...
		ret = -95; //WIFI_ERROR_NOT_SUPPORTED=-3
		goto exit;
	}
...
static int
wl_cfgvendor_google_set_p2p_rand_mac(struct wiphy *wiphy,
		struct wireless_dev *wdev, const void  *data, int len)
{
	#ifdef WL_P2P_RAND
		return wl_cfgvendor_set_p2p_rand_mac(wiphy, wdev, data, len);
	#else
		return 0;
	#endif /* WL_P2P_RAND */
}
...
static struct wiphy_vendor_command wl_vendor_cmds [] = {
...
#endif /* WL_P2P_RAND */
	{
		...
		.flags = WIPHY_VENDOR_CMD_NEED_WDEV,
		.doit = wl_cfgvendor_google_set_p2p_rand_mac,
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(5, 3, 0))
		.policy = brcm_drv_attr_policy,
		.maxattr = BRCM_ATTR_DRIVER_MAX
#endif /* LINUX_VERSION >= 5.3 */
