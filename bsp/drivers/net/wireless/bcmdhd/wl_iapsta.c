...
static void wl_tput_uevent(struct dhd_pub *dhd)
{
	char *envp[] = {
		"SYSTEM=WIFI",
		"EVENT=BOOSTREQ",
	...
	struct device *dhd_dev = dhd_bus_to_dev(dhd->bus);

	if (dhd_dev)
		kobject_uevent_env(&dhd_dev->kobj, KOBJ_CHANGE, envp);
}
	...
	const wl_event_msg_t *e, void *data)
{
	...
	if (etype == WLC_E_RESERVED && reason == ISAM_RC_TPUT_MONITOR) {
		...
		if (apsta_params->tput_sum >= 100) {
			/* tput uevent request to userspace when tput >= 100Mbps */
			wl_tput_uevent(dhd);
		}
