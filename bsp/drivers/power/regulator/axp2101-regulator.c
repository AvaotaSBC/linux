...
static int regulator_is_enabled_regmap_axp2202_c_drivevbus(struct regulator_dev *rdev)
{
	...
	ret = regmap_read(rdev->regmap, AXP2202_RBFET_CTRL, &val[0]);
	if (ret)
		return ret;

	ret = regmap_read(rdev->regmap, AXP2202_MODULE_EN, &val[1]);
	if (ret)
		return ret;

	return ((val[0] &= BIT(0)) && (val[1] &= BIT(4)));
}

static int regulator_enable_regmap_axp2202_c_drivevbus(struct regulator_dev *rdev)
{
	int ret;

	ret = regmap_update_bits(rdev->regmap, AXP2202_RBFET_CTRL, BIT(0), BIT(0));
	if (ret)
		return ret;

	ret = regmap_update_bits(rdev->regmap, AXP2202_MODULE_EN, BIT(4), BIT(4));

	return ret;
}

static int regulator_disable_regmap_axp2202_c_drivevbus(struct regulator_dev *rdev)
{
	int ret;

	ret = regmap_update_bits(rdev->regmap, AXP2202_RBFET_CTRL, BIT(0), 0);
	if (ret)
		return ret;

	ret = regmap_update_bits(rdev->regmap, AXP2202_MODULE_EN, BIT(4), 0);

	return ret;
}

static int regulator_disable_regmap_axp2202_rbfet(struct regulator_dev *rdev)
{
	int ret = 0;
	ret = regmap_update_bits(rdev->regmap, AXP2202_IRQ_EN1,  0xC0, 0);
	if (ret)
		return ret;
	...
	ret = regmap_update_bits(rdev->regmap, AXP2202_IRQ1, 0xC0, 0xC0);
	if (ret)
		return ret;
	...
	if (ret)
		return ret;
	ret = regmap_update_bits(rdev->regmap, AXP2202_IRQ_EN1, 0xC0, 0xC0);
...
static int axp_set_voltage_time(struct regulator_dev *rdev,
				       int old_uV, int new_uV)
{
	...
	if (ramp_delay == 0) {
		pr_debug("ramp_delay not set\n");
		return 0;
	}

	delay = (DIV_ROUND_UP(abs(new_uV - old_uV), ramp_delay) + 999) / 1000;
	return delay * 1000;
}
...
static struct regulator_ops axp20x_ops_range_vol_delay = {
	...
	.disable		= regulator_disable_regmap,
	.is_enabled		= regulator_is_enabled_regmap,
	.set_voltage_time       = axp_set_voltage_time,
};
...
static struct regulator_ops axp2202_c_ops_drivevbus = {
	.enable			= regulator_enable_regmap_axp2202_c_drivevbus,
	.disable		= regulator_disable_regmap_axp2202_c_drivevbus,
	.is_enabled		= regulator_is_enabled_regmap_axp2202_c_drivevbus,
};

static struct regulator_ops axp2202_ops_rbfet = {
	.enable			= regulator_enable_regmap,
	.disable		= regulator_disable_regmap_axp2202_rbfet,
	.is_enabled		= regulator_is_enabled_regmap,
};
static struct regulator_ops axp2202_c_ops_vmid;
...
static const struct linear_range axp22x_dcdc1_ranges[] = {
	REGULATOR_LINEAR_RANGE(1600000, 0x0, 0x12, 100000),
};

static const struct linear_range axp22x_dcdc2_ranges[] = {
	REGULATOR_LINEAR_RANGE(600000, 0x0, 0x30, 20000),
};

static const struct linear_range axp22x_dcdc3_ranges[] = {
	REGULATOR_LINEAR_RANGE(600000, 0x0, 0x40, 20000),
};

static const struct linear_range axp22x_dcdc4_ranges[] = {
	REGULATOR_LINEAR_RANGE(600000, 0x0, 0x30, 20000),
};

static const struct linear_range axp22x_dcdc5_ranges[] = {
	REGULATOR_LINEAR_RANGE(1000000, 0x0, 0x20, 50000),
};

static const struct linear_range axp22x_dc5ldo_ranges[] = {
	REGULATOR_LINEAR_RANGE(1100000, 0x0, 0x00, 0),
	REGULATOR_LINEAR_RANGE(800000, 0x1, 0x08, 100000),
};

static const struct regulator_desc axp22x_regulators[] = {
	AXP_DESC_RANGES_VOL_DELAY
		(AXP22X, DCDC1, "dcdc1", "vin1", axp22x_dcdc1_ranges,
		0x13, AXP22X_DCDC1_V_OUT, 0x1f, AXP22X_PWR_OUT_CTRL1, BIT(1)),
	AXP_DESC_RANGES
		(AXP22X, DCDC2, "dcdc2", "vin2", axp22x_dcdc2_ranges,
		0x31, AXP22X_DCDC2_V_OUT, 0x3f, AXP22X_PWR_OUT_CTRL1, BIT(2)),
	AXP_DESC_RANGES_VOL_DELAY
		(AXP22X, DCDC3, "dcdc3", "vin3", axp22x_dcdc3_ranges,
		0x41, AXP22X_DCDC3_V_OUT, 0x3f, AXP22X_PWR_OUT_CTRL1, BIT(3)),
	AXP_DESC_RANGES_VOL_DELAY
		(AXP22X, DCDC4, "dcdc4", "vin4", axp22x_dcdc4_ranges,
		0x31, AXP22X_DCDC4_V_OUT, 0x3f, AXP22X_PWR_OUT_CTRL1, BIT(4)),
	AXP_DESC_RANGES_VOL_DELAY
		(AXP22X, DCDC5, "dcdc5", "vin5", axp22x_dcdc5_ranges,
		0x21, AXP22X_DCDC5_V_OUT, 0x1f, AXP22X_PWR_OUT_CTRL1, BIT(5)),
	/* secondary switchable output of DCDC1 */
	AXP_DESC_SW(AXP22X, DC1SW, "dc1sw", NULL, AXP22X_PWR_OUT_CTRL2,
		    BIT(7)),
	/* LDO regulator internally chained to DCDC5 */
	AXP_DESC_RANGES_VOL_DELAY
		(AXP22X, DC5LDO, "dc5ldo", NULL, axp22x_dc5ldo_ranges,
		0x09, AXP22X_DC5LDO_V_OUT, 0x7, AXP22X_PWR_OUT_CTRL1, BIT(0)),
	...
	AXP_DESC_IO(AXP22X, LDO_IO0, "ldo_io0", "ips", 700, 3300, 100,
		    AXP22X_LDO_IO0_V_OUT, 0x1a, AXP20X_GPIO0_CTRL, 0x07,
		    AXP22X_IO_ENABLED, AXP22X_IO_DISABLED),
	/* Note the datasheet only guarantees reliable operation up to
	 * 3.3V, this needs to be enforced via dts provided constraints */
	AXP_DESC_IO(AXP22X, LDO_IO1, "ldo_io1", "ips", 700, 3300, 100,
		    AXP22X_LDO_IO1_V_OUT, 0x1a, AXP20X_GPIO1_CTRL, 0x07,
		    AXP22X_IO_ENABLED, AXP22X_IO_DISABLED),
...
static struct linear_range axp2202_dcdc1_ranges[] = {
	REGULATOR_LINEAR_RANGE(500000, 0x0, 0x46, 10000),
	REGULATOR_LINEAR_RANGE(1220000, 0x47, 0x57, 20000),
};

static struct linear_range axp2202_dcdc2_ranges[] = {
	REGULATOR_LINEAR_RANGE(500000, 0, 0x46, 10000),
	REGULATOR_LINEAR_RANGE(1220000, 0x47, 0x57, 20000),
	REGULATOR_LINEAR_RANGE(1600000, 0x58, 0x6a, 100000),
};

static struct linear_range axp2202_dcdc3_ranges[] = {
	REGULATOR_LINEAR_RANGE(500000, 0, 0x46, 10000),
	REGULATOR_LINEAR_RANGE(1220000, 0x47, 0x66, 20000),
};
...
static struct linear_range axp8191_dcdc1_ranges[] = {
	REGULATOR_LINEAR_RANGE(1000000, 0x0, 0x1c, 100000),
};

static struct linear_range axp8191_dcdc2_ranges[] = {
	REGULATOR_LINEAR_RANGE(500000, 0x0, 0x46, 10000),
	REGULATOR_LINEAR_RANGE(1220000, 0x47, 0x57, 20000),
};

static struct linear_range axp8191_dcdc3_ranges[] = {
	REGULATOR_LINEAR_RANGE(500000, 0x0, 0x46, 10000),
	REGULATOR_LINEAR_RANGE(1220000, 0x47, 0x57, 20000),
};

static struct linear_range axp8191_dcdc4_ranges[] = {
	REGULATOR_LINEAR_RANGE(500000, 0x0, 0x46, 10000),
	REGULATOR_LINEAR_RANGE(1220000, 0x47, 0x57, 20000),
};

static struct linear_range axp8191_dcdc5_ranges[] = {
	REGULATOR_LINEAR_RANGE(500000, 0x0, 0x46, 10000),
	REGULATOR_LINEAR_RANGE(1220000, 0x47, 0x57, 20000),
};

static struct linear_range axp8191_dcdc6_ranges[] = {
	REGULATOR_LINEAR_RANGE(500000, 0x0, 0x46, 10000),
	REGULATOR_LINEAR_RANGE(1220000, 0x47, 0x57, 20000),
	REGULATOR_LINEAR_RANGE(1800000, 0x58, 0x67, 20000),
	REGULATOR_LINEAR_RANGE(2440000, 0x68, 0x70, 40000),
};

static struct linear_range axp8191_dcdc7_ranges[] = {
	REGULATOR_LINEAR_RANGE(500000, 0x0, 0x46, 10000),
	REGULATOR_LINEAR_RANGE(1220000, 0x47, 0x57, 20000),
};

static struct linear_range axp8191_dcdc8_ranges[] = {
	REGULATOR_LINEAR_RANGE(500000, 0x0, 0x46, 10000),
	REGULATOR_LINEAR_RANGE(1220000, 0x47, 0x57, 20000),
	REGULATOR_LINEAR_RANGE(1900000, 0x58, 0x67, 100000),
};

static struct linear_range axp8191_dcdc9_ranges[] = {
	REGULATOR_LINEAR_RANGE(500000, 0x0, 0x46, 10000),
	REGULATOR_LINEAR_RANGE(1220000, 0x47, 0x57, 20000),
	REGULATOR_LINEAR_RANGE(1900000, 0x58, 0x67, 100000),
};

static const struct regulator_desc axp8191_regulators[] = {
	AXP_DESC_RANGES_VOL_DELAY
			(AXP8191, DCDC1, "dcdc1", "vin-ps", axp8191_dcdc1_ranges,
			0x1d, AXP8191_DC1OUT_VOL, GENMASK(4, 0),
			AXP8191_DCDC_CTL1, BIT(0)),
	AXP_DESC_RANGES_VOL_DELAY
			(AXP8191, DCDC2, "dcdc2", "vin-ps", axp8191_dcdc2_ranges,
			0x58, AXP8191_DC2OUT_VOL, GENMASK(6, 0),
			AXP8191_DCDC_CTL1, BIT(1)),
	AXP_DESC_RANGES_VOL_DELAY
			(AXP8191, DCDC3, "dcdc3", "vin-ps", axp8191_dcdc3_ranges,
			0x58, AXP8191_DC3OUT_VOL, GENMASK(6, 0),
			AXP8191_DCDC_CTL1, BIT(2)),
	AXP_DESC_RANGES_VOL_DELAY
			(AXP8191, DCDC4, "dcdc4", "vin-ps", axp8191_dcdc4_ranges,
			0x58, AXP8191_DC4OUT_VOL, GENMASK(6, 0),
			AXP8191_DCDC_CTL1, BIT(3)),
	AXP_DESC_RANGES_VOL_DELAY
			(AXP8191, DCDC5, "dcdc5", "vin-ps", axp8191_dcdc5_ranges,
			0x58, AXP8191_DC5OUT_VOL, GENMASK(6, 0),
			AXP8191_DCDC_CTL1, BIT(4)),
	AXP_DESC_RANGES_VOL_DELAY
			(AXP8191, DCDC6, "dcdc6", "vin-ps", axp8191_dcdc6_ranges,
			0x71, AXP8191_DC6OUT_VOL, GENMASK(6, 0),
			AXP8191_DCDC_CTL1, BIT(5)),
	AXP_DESC_RANGES_VOL_DELAY
			(AXP8191, DCDC7, "dcdc7", "vin-ps", axp8191_dcdc7_ranges,
			0x58, AXP8191_DC7OUT_VOL, GENMASK(6, 0),
			AXP8191_DCDC_CTL1, BIT(6)),
	AXP_DESC_RANGES_VOL_DELAY
			(AXP8191, DCDC8, "dcdc8", "vin-ps", axp8191_dcdc8_ranges,
			0x69, AXP8191_DC8OUT_VOL, GENMASK(6, 0),
			AXP8191_DCDC_CTL1, BIT(7)),
	AXP_DESC_RANGES_VOL_DELAY
			(AXP8191, DCDC9, "dcdc9", "vin-ps", axp8191_dcdc9_ranges,
			0x68, AXP8191_DC9OUT_VOL, GENMASK(6, 0),
			AXP8191_DCDC_CTL2, BIT(0)),
	AXP_DESC_SW(AXP8191, DC1SW1, "dc1sw1", "dcdc1", AXP8191_DCDC_CTL2,
		    BIT(3)),
	AXP_DESC_SW(AXP8191, DC1SW2, "dc1sw2", "dcdc1", AXP8191_DCDC_CTL2,
		    BIT(4)),
	AXP_DESC(AXP8191, ALDO1, "aldo1", "aldo", 500, 3400, 100,
		 AXP8191_ALDO1OUT_VOL, GENMASK(4, 0), AXP8191_LDO_POWER_ON_OFF_CTL1,
		 BIT(0)),
	AXP_DESC(AXP8191, ALDO2, "aldo2", "aldo", 500, 3400, 100,
		 AXP8191_ALDO2OUT_VOL, GENMASK(4, 0), AXP8191_LDO_POWER_ON_OFF_CTL1,
		 BIT(1)),
	AXP_DESC(AXP8191, ALDO3, "aldo3", "aldo", 500, 3400, 100,
		 AXP8191_ALDO3OUT_VOL, GENMASK(4, 0), AXP8191_LDO_POWER_ON_OFF_CTL1,
		 BIT(2)),
	AXP_DESC(AXP8191, ALDO4, "aldo4", "aldo", 500, 3400, 100,
		 AXP8191_ALDO4OUT_VOL, GENMASK(4, 0), AXP8191_LDO_POWER_ON_OFF_CTL1,
		 BIT(3)),
	AXP_DESC(AXP8191, ALDO5, "aldo5", "aldo", 500, 3400, 100,
		 AXP8191_ALDO5OUT_VOL, GENMASK(4, 0), AXP8191_LDO_POWER_ON_OFF_CTL1,
		 BIT(4)),
	AXP_DESC(AXP8191, ALDO6, "aldo6", "aldo", 500, 3400, 100,
		 AXP8191_ALDO6OUT_VOL, GENMASK(4, 0), AXP8191_LDO_POWER_ON_OFF_CTL1,
		 BIT(5)),

	AXP_DESC(AXP8191, BLDO1, "bldo1", "bldo", 500, 3400, 100,
		 AXP8191_BLDO1OUT_VOL, GENMASK(4, 0), AXP8191_LDO_POWER_ON_OFF_CTL1,
		 BIT(6)),
	AXP_DESC(AXP8191, BLDO2, "bldo2", "bldo", 500, 3400, 100,
		 AXP8191_BLDO2OUT_VOL, GENMASK(4, 0), AXP8191_LDO_POWER_ON_OFF_CTL1,
		 BIT(7)),
	AXP_DESC(AXP8191, BLDO3, "bldo3", "bldo", 500, 3400, 100,
		 AXP8191_BLDO3OUT_VOL, GENMASK(4, 0), AXP8191_LDO_POWER_ON_OFF_CTL2,
		 BIT(0)),
	AXP_DESC(AXP8191, BLDO4, "bldo4", "bldo", 500, 3400, 100,
		 AXP8191_BLDO4OUT_VOL, GENMASK(4, 0), AXP8191_LDO_POWER_ON_OFF_CTL2,
		 BIT(1)),
	AXP_DESC(AXP8191, BLDO5, "bldo5", "bldo", 500, 3400, 100,
		 AXP8191_BLDO5OUT_VOL, GENMASK(4, 0), AXP8191_LDO_POWER_ON_OFF_CTL2,
		 BIT(2)),

	AXP_DESC(AXP8191, CLDO1, "cldo1", "cldo", 500, 3400, 100,
		 AXP8191_CLDO1OUT_VOL, GENMASK(4, 0), AXP8191_LDO_POWER_ON_OFF_CTL2,
		 BIT(3)),
	AXP_DESC(AXP8191, CLDO2, "cldo2", "cldo", 500, 3400, 100,
		 AXP8191_CLDO2OUT_VOL, GENMASK(4, 0), AXP8191_LDO_POWER_ON_OFF_CTL2,
		 BIT(4)),
	AXP_DESC(AXP8191, CLDO3, "cldo3", "cldo", 500, 3400, 100,
		 AXP8191_CLDO3OUT_VOL, GENMASK(4, 0), AXP8191_LDO_POWER_ON_OFF_CTL2,
		 BIT(5)),
	AXP_DESC(AXP8191, CLDO4, "cldo4", "cldo", 500, 3400, 100,
		 AXP8191_CLDO4OUT_VOL, GENMASK(4, 0), AXP8191_LDO_POWER_ON_OFF_CTL2,
		 BIT(6)),
	AXP_DESC(AXP8191, CLDO5, "cldo5", "cldo", 500, 3400, 100,
		 AXP8191_CLDO5OUT_VOL, GENMASK(4, 0), AXP8191_LDO_POWER_ON_OFF_CTL2,
		 BIT(7)),

	AXP_DESC(AXP8191, DLDO1, "dldo1", "dldo", 500, 3400, 100,
		 AXP8191_DLDO1OUT_VOL, GENMASK(4, 0), AXP8191_LDO_POWER_ON_OFF_CTL3,
		 BIT(0)),
	AXP_DESC(AXP8191, DLDO2, "dldo2", "dldo", 500, 3400, 100,
		 AXP8191_DLDO2OUT_VOL, GENMASK(4, 0), AXP8191_LDO_POWER_ON_OFF_CTL3,
		 BIT(1)),
	AXP_DESC(AXP8191, DLDO3, "dldo3", "dldo", 500, 3400, 100,
		 AXP8191_DLDO3OUT_VOL, GENMASK(4, 0), AXP8191_LDO_POWER_ON_OFF_CTL3,
		 BIT(2)),
	AXP_DESC(AXP8191, DLDO4, "dldo4", "dldo", 500, 3400, 100,
		 AXP8191_DLDO4OUT_VOL, GENMASK(4, 0), AXP8191_LDO_POWER_ON_OFF_CTL3,
		 BIT(3)),
	AXP_DESC(AXP8191, DLDO5, "dldo5", "dldo", 500, 3400, 100,
		 AXP8191_DLDO5OUT_VOL, GENMASK(4, 0), AXP8191_LDO_POWER_ON_OFF_CTL3,
		 BIT(4)),
	AXP_DESC(AXP8191, DLDO6, "dldo6", "dldo", 500, 3400, 100,
		 AXP8191_DLDO6OUT_VOL, GENMASK(4, 0), AXP8191_LDO_POWER_ON_OFF_CTL3,
		 BIT(5)),

	AXP_DESC(AXP8191, ELDO1, "eldo1", "eldo", 500, 1500, 25,
		 AXP8191_ELDO1OUT_VOL, GENMASK(5, 0), AXP8191_LDO_POWER_ON_OFF_CTL3,
		 BIT(6)),
	AXP_DESC(AXP8191, ELDO2, "eldo2", "eldo", 500, 1500, 25,
		 AXP8191_ELDO2OUT_VOL, GENMASK(5, 0), AXP8191_LDO_POWER_ON_OFF_CTL3,
		 BIT(7)),
	AXP_DESC(AXP8191, ELDO3, "eldo3", "eldo", 500, 1500, 25,
		 AXP8191_ELDO3OUT_VOL, GENMASK(5, 0), AXP8191_LDO_POWER_ON_OFF_CTL4,
		 BIT(0)),
	AXP_DESC(AXP8191, ELDO4, "eldo4", "eldo", 500, 1500, 25,
		 AXP8191_ELDO4OUT_VOL, GENMASK(5, 0), AXP8191_LDO_POWER_ON_OFF_CTL4,
		 BIT(1)),
	AXP_DESC(AXP8191, ELDO5, "eldo5", "eldo", 500, 1500, 25,
		 AXP8191_ELDO5OUT_VOL, GENMASK(5, 0), AXP8191_LDO_POWER_ON_OFF_CTL4,
		 BIT(2)),
	AXP_DESC(AXP8191, ELDO6, "eldo6", "eldo", 500, 1500, 25,
		 AXP8191_ELDO6OUT_VOL, GENMASK(5, 0), AXP8191_LDO_POWER_ON_OFF_CTL4,
		 BIT(3)),
	AXP_DESC_FIXED(AXP8191, RTCLDO, "rtcldo", "vin-ps", 1800),
};
...
static const struct regulator_desc axp2202_vmid_regulator = {
	.name		= "vmid",
	.supply_name	= "vin-ps",
...
static const struct regulator_desc axp2202_drivevbus_regulator = {
	...
	.enable_mask	= BIT(0),
	.ops		= &axp2202_ops_rbfet,
};

static const struct regulator_desc axp2202_c_vmid_regulator = {
	.name		= "vmid",
	.supply_name	= "vin-ps",
	.of_match	= of_match_ptr("vmid"),
	.regulators_node = of_match_ptr("regulators"),
	.type		= REGULATOR_VOLTAGE,
	.owner		= THIS_MODULE,
	.ops		= &axp2202_c_ops_vmid,
};

static const struct regulator_desc axp2202_c_drivevbus_regulator = {
	.name		= "drivevbus",
	.supply_name	= "drivevbusin",
	.of_match	= of_match_ptr("drivevbus"),
	.regulators_node = of_match_ptr("regulators"),
...
static int axp20x_regulator_parse_dt(struct platform_device *pdev)
{
	...
	if (!regulators) {
		...
		if (ret < 0) {
			PMIC_DEV_ERR(&pdev->dev, "Error setting dcdc frequency: %d\n", ret);
			return ret;
		}
...
static int axp2101_regulator_probe(struct platform_device *pdev)
{
	...
	case AXP858_ID:
		...
		break;
	case AXP8191_ID:
		regulators = axp8191_regulators;
		nregulators = AXP8191_REG_ID_MAX;
		break;
	case AXP803_ID:
	...
	break;
	default:
		PMIC_DEV_ERR(&pdev->dev, "Unsupported AXP variant: %ld\n",
			axp20x->variant);
		return -EINVAL;
	}
	...
	for (i = 0; i < nregulators; i++) {
		...
		if ((regulators == axp22x_regulators && i == AXP22X_DC1SW) ||
			...
			new_desc = devm_kzalloc(&pdev->dev, sizeof(*desc),
						GFP_KERNEL);
			if (!new_desc) {
				PMIC_DEV_ERR(&pdev->dev, "new_desc kzalloc erroe %s\n", regulators[i].name);
				return -ENOMEM;
			}
		...
		if (!ret) {
			if (axp20x_set_dcdc_workmode(rdev, i, workmode))
				PMIC_DEV_ERR(&pdev->dev, "Failed to set workmode on %s\n",
					rdev->desc->name);
		}
	...
	if (drivevbus) {
		...
		case AXP803_ID:
			...
			break;
		case AXP2202_ID:
			regmap_read(axp20x->regmap, AXP2202_VBUS_TYPE, &reg);
			if (!reg) {
				rdev = devm_regulator_register(&pdev->dev,
						       &axp2202_c_vmid_regulator,
						       &config);

				rdev = devm_regulator_register(&pdev->dev,
						       &axp2202_c_drivevbus_regulator,
						       &config);
			} else {
				/* except version c, use real vmid regulator */
				rdev = devm_regulator_register(&pdev->dev,
						       &axp2202_vmid_regulator,
						       &config);
