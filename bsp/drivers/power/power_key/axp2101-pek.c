...
static int axp22x_config_set(struct axp20x_pek *axp20x_pek)
{
	...
	/* onlevel setting */
	regmap_read(regmap, AXP22X_POK_SET, &val);
	if (pk_dts->pmu_powkey_on_time <= 128)
		val &= 0x3f;
	else if (pk_dts->pmu_powkey_on_time <= 1000) {
		val &= 0x3f;
	...
	regmap_write(regmap, AXP22X_POK_SET, val);
...
static int axp20x_pek_probe(struct platform_device *pdev)
{
	...
	if (!axp20x->irq) {
		PMIC_ERR("axp2101-pek can not register without irq\n");
		return -EINVAL;
	}
	...
	error = devm_request_any_context_irq(&pdev->dev, axp20x_pek->irq_dbr,
					     ...
					     "axp20x-pek-dbr", idev);
	if (error < 0) {
		PMIC_DEV_ERR(axp20x->dev, "Failed to request dbr IRQ#%d: %d\n",
	...
	error = devm_request_any_context_irq(&pdev->dev, axp20x_pek->irq_dbf,
					  ...
					  "axp20x-pek-dbf", idev);
	if (error < 0) {
		PMIC_DEV_ERR(axp20x->dev, "Failed to request dbf IRQ#%d: %d\n",
	...
	if (error) {
		PMIC_DEV_ERR(axp20x->dev, "Can't register input device: %d\n",
			error);
		return error;
	}
