...
#define SUNXI_NOR_MODULE_VERSION	"1.0.1"
...
static ssize_t spinordbg_status_read(struct file *file, char __user *buf,
				    size_t count, loff_t *ppos)
{
	struct spi_nor *nor = spinordbg;
	char tmpstatus[80];
	u32 value = 0;
	int len = 0;

	if (!strncmp(spinordbg_priv.param, "freq", 4)) {
		if (!strncmp(nor->mtd.name, "spif", 4))
			of_property_read_u32(nor->dev->of_node,
		...
		case SPINOR_OP_READ_1_4_4_DTR_4B:
			strcpy(spinordbg_priv.status, "QUAD-DTR");
			break;
		...
		case SPINOR_OP_READ_1_2_2_DTR_4B:
			strcpy(spinordbg_priv.status, "DUAL-DTR");
			break;
...
{
	struct spi_nor *nor = spinordbg;
	int read_cmd = SNOR_CMD_READ_MAX, pp_cmd = SNOR_CMD_PP_MAX;
	const struct spi_nor_read_command *read;
	const struct spi_nor_pp_command *pp;

	if (copy_from_user(spinordbg_priv.status, buf, count)) {
		pr_warn("copy_from_user fail\n");
		return 0;
	}

	if (!strncmp(spinordbg_priv.status, "1-1-1", 5)) {
		read_cmd = SNOR_CMD_READ_FAST;
		pp_cmd = SNOR_CMD_PP;
	} else if (!strncmp(spinordbg_priv.status, "1-1-2", 5)) {
		read_cmd = SNOR_CMD_READ_1_1_2;
		pp_cmd = SNOR_CMD_PP_MAX;
	} else if (!strncmp(spinordbg_priv.status, "1-1-4", 5)) {
		read_cmd = SNOR_CMD_READ_1_1_4;
		pp_cmd = SNOR_CMD_PP_1_1_4;
	} else {
		dev_warn(nor->dev, "please set param before echo x-x-x > status\n \
	...
	if (read_cmd != SNOR_CMD_READ_MAX) {
		read = &nor->params->reads[read_cmd];
	...
	if (pp_cmd != SNOR_CMD_PP_MAX) {
		pp = &nor->params->page_programs[pp_cmd];

		if (nor->program_opcode) {
			nor->program_opcode = pp->opcode;
			nor->write_proto = pp->proto;
		}
	}
...
int spinor_debug_init(void)
{
	...
	my_spinordbg_root = debugfs_create_dir("spinor", NULL);
